...
| Конечная точка | `/v2/payment/wallet/paymaya/sale` |
...
| returntype_string type_optional | **return_url**`https://example.com/return/`Пример:URL-адрес, куда нужно перенаправить пользователя в случае прерывания оплаты пользователем до ее завершения. Если не задан ни один из параметров объекта, то Gate по умолчанию перенаправляет пользователя по URL-адресу, указанному в вашем проекте в Some Platform. |  |
...
| --- | --- |
| Формат тела запроса | JSON |
| Конечная точка | `/v2/payment/wallet/paymaya/payout` |
| Полная спецификация конечной точки |  |
...
| last_nametype_string type_strictly_required | `Doe`Пример:Фамилия пользователя. |  |
| emailtype_string type_strictly_required | `johndoe@example.com`Пример:Адрес электронной почты пользователя. |  |
| accounttype_object type_strictly_required | numbertype_string type_strictly_required | `09``00000000000`Пример:Идентификатор учетной записи, должен состоять из 11 цифр и начинаться с. |
| paymenttype_object type_strictly_required | currencytype_string type_strictly_required | `example_currency`Пример:Код валюты платежа в формате ISO-4217 alpha-3. |
| amounttype_integer type_strictly_required | *за исключением*Сумма платежа в дробных единицах валюты без десятичного разделителя (точки или запятой) и пробеловслучаев, когда у валюты нет дробной части.*основных единицах валюты*ru_currency_units.ditaru_currency_codes.ditaЕсли у валюты нет дробных единиц (то есть количество разрядов дробных единиц равно нулю), то в этом параметре нужно указывать сумму в. Подробнее о разрядах дробных единиц у валют см.и.`example_amount`Пример: example_amount_in_majors example_currency передается как |  |
| descriptiontype_string type_strictly_required | `Description`Пример:Описание или комментарий к операции. |  |
| API ReferenceПри необходимости добавьте в запрос необязательные параметры, указанные в спецификации                     Gate:. |  |  |
...
| **Особенности**ru_pm_coinsph.ditaru_pm_gcash.ditaru_pm_grabpay.ditaru_pm_paymaya.ditaПроведение выплаты какому-либо пользователю допустимо только после проведения оплаты этим пользователем одним из следующих платежных методов:,,,.InstaPayPESONet**Время зачисления платежа**С минимальной задержкойЗапросы, поступившие в систему с 9:31 до 15:30, обрабатываются до 22:00. Запросы, поступившие в систему с 15:31 текущего дня до 9:30 следующего дня, обрабатываются до 13:00 следующего дня. Все времена указаны для зоны UTC+8.Подробности о проведении платежей через PESONet и Instapay уточняйте у своего курирующего менеджера Some Platform.В некоторых случаях платежи проводятся через сервисы InstaPay и PESONet. Вот основные различия этих сервисов: |  |
...
| **Возврат** |  |
| **Организация и стоимость подключения** | По согласованию с курирующим менеджером Some Platform |
| **Особенности**InstaPayPESONet**Время зачисления платежа**С минимальной задержкойЗапросы, поступившие в систему с 9:31 до 15:30, обрабатываются до 22:00. Запросы, поступившие в систему с 15:31 текущего дня до 9:30 следующего дня, обрабатываются до 13:00 следующего дня. Все времена указаны для зоны UTC+8.В некоторых случаях платежи проводятся через сервисы InstaPay и PESONet. Вот основные различия этих сервисов:Подробности о проведении платежей через PESONet и Instapay уточняйте у своего курирующего менеджера Some Platform. |  |
