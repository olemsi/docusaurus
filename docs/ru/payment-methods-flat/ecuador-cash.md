...
| Конечная точка | `/v2/payment/cash/ecuador/sale` |
...
| phonetype_string type_required | `1234567`Пример:Номер телефона пользователя. |  |
| identifytype_object type_required | doc_numbertype_string type_required | doc_type`1234567890`Пример:Номер документа, подтверждающего личность пользователя. Номер документа должен соответствовать типу, передаваемому в параметре. |
| doc_typetype_string type_required | Возможные значения:`CI`Пример:`PAS`— при передаче номера паспорта;`CI`— при передаче идентификатора удостоверения личности (Cédula de Identidad);`RUC`— при передаче идентификатора налогоплательщика в Эквадоре (Registro Único de Contribuyente).Тип документа, подтверждающего личность пользователя. |  |
...
| customer_phonetype_string type_required | `593123456789`Пример:Номер телефона пользователя, без знака +, без пробелов и знаков препинания. |
| identify_doc_numbertype_string type_required | doc_type`1234567890`Пример:Номер документа, подтверждающего личность пользователя. Номер документа должен соответствовать типу, передаваемому в параметре. |
...
| **Особенности**Реквизиты для оплаты должны использоваться только один раз. Для повторной оплаты пользователь должен инициировать новый платеж.Платеж отклоняется, если пользователь не совершит оплату в течение определенного времени с момента инициирования платежа. |  |
