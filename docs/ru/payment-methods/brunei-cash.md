...
| Конечная точка | `v2/payment/cash/brunei/sale` |
...
| paymenttype_object type_strictly_required | currencytype_string type_strictly_required | references/ru/currencies/BND.ditaВ качестве валюты оплаты можно использовать только.`example_currency`Пример:Код валюты платежа в формате ISO-4217 alpha-3. |
...
| --- | --- |
| Формат тела запроса | JSON |
| Конечная точка для выплаты в точке выдачи наличных | `v2/payment/cash/brunei/payout` |
| Конечная точка для выплаты посредством доставки наличных | `v2/payment/cash/delivery/payout` |
| Полная спецификация конечной точки |  |
...
| last_nametype_string type_strictly_required | `Doe`Пример:Фамилия пользователя. |  |
| emailtype_string type_strictly_required | `johndoe@example.com`Пример:Адрес электронной почты пользователя. |  |
| phonetype_string type_strictly_required | `customer.account_id``6731234567`Пример:Номер телефона пользователя. Состоит из кода страны 673 и 7 цифр. Если этот параметр передан, то параметрнеобязателен. |  |
| account_idtype_string type_strictly_required | `customer.phone``A123456`Пример:Аккаунт пользователя в мессенджере Telegram. Может включать до 30 символов. Если этот параметр передан, то параметрнеобязателен. |  |
| paymenttype_object type_strictly_required | currencytype_string type_strictly_required | references/ru/currencies/BND.ditaВ качестве валюты оплаты можно использовать только.`example_currency`Пример:Код валюты платежа в формате ISO-4217 alpha-3. |
...
| descriptiontype_string type_strictly_required | `Описание платежа`Пример:Описание платежа, в котором нужно передавать номер купюры. Может состоять из 6-15 символов, включая только цифры и латинские буквы. |  |
| API ReferenceПри необходимости добавьте в запрос необязательные параметры, указанные в спецификации Gate:. |  |  |
...
| --- | --- |
| **Валюты выплаты** | references/ru/currencies/BND.dita |
...
| **Особенности**references/ru/currencies/BND.ditaВ качестве валюты оплаты можно использовать только.Получение выплаты в этом методе возможно как на точке получения наличных, так и с помощью доставки. Выбор способа получения наличных возможен с помощью двух конечных точек. |  |
...
| payment_currencytype_string type_strictly_required | references/ru/currencies/BND.ditaВ качестве валюты оплаты можно использовать только.`example_currency`Пример:Код валюты платежа в формате ISO-4217 alpha-3. |
...
| --- | --- |
| Регионы использования |  |
| Валюты платежей | references/ru/currencies/BND.dita |
| Суммы платежей | Информацию уточняйте у курирующего менеджера Some Platform. Также вы можете уточнить в Dashboard минимальную и максимальную сумму платежа, доступную в вашем проекте. |
| Время проведения платежа | Информацию уточняйте у курирующего менеджера Some Platform. |
| Конвертация валют |  |
| Оплата |  |
