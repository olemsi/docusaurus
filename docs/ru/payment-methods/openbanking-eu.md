...
| Конечная точка | `/v2/payment/banks/sepa/sale` |
...
| countrytype_string type_strictly_required | Коды стран`PL`Пример:Код страны пользователя. Подробнее о стране см.. |  |
| accounttype_object type_strictly_required | numbertype_string type_required | `AB123456789012345678`Пример:Номер счёта (International Bank Account Number, IBAN). |
| bank_idtype_strictly_required✱type_string | ru_pm_openbanking_eu_purchase_redirect_banks.dita`22411`Пример:Идентификатор банка пользователя. Подробнее о том, как получить список поддерживаемых банков см.. |  |
...
| account_numbertype_string type_required | `AB123456789012345678`Пример:Номер счёта (International Bank Account Number, IBAN). |
...
| customer_countrytype_string type_strictly_required | Коды стран`PL`Пример:Код страны пользователя. Подробнее о стране см.. |
...
| **Регионы использования** | country_AT, country_BE, country_BG, country_GB, country_HU, country_DE, country_DK, country_IE, country_ES, country_IT, country_LV, country_LT, country_LU, country_NL, country_NO, country_PL, country_PT, country_RO, country_FI, country_FR, country_CZ, country_SE, country_EE. |
| --- | --- |
| **Валюты оплаты** | references/ru/currencies/EUR.ditareferences/ru/currencies/GBP.ditareferences/ru/currencies/PLN.dita,, |
...
Важно передавать реальные данные о платеже, но если платеж еще не сформирован, в качестве идентификатора платежа в запросе можно указать случайное значение.
...
| --- | --- |
| **Формат тела запроса** | JSON |
| **Конечная точка** | `/v2/info/banks/sepa/sale/list` |
| **Полная спецификация конечной точки** |  |
...
| customertype_object type_strictly_required | countrytype_string type_strictly_required | ru_country_codes.dita`example_currency`Пример:Код страны пользователя. Подробнее о стране см.. |
| API ReferenceПри необходимости добавьте в запрос необязательные параметры, указанные в спецификации Gate:. |  |  |
