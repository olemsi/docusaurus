...
| Конечная точка | `/v2/payment/wallet/orange/sale` |
...
| {     "type": "add_info",     "title": "currency",     "data": "MAD" } | `MAD`Пример:Код валюты оплаты в формате ISO-4217 alpha-3. |
| {     "type": "add_info",     "title": "receiver_account",     "data": "+212123456789012" } | `+212123456789012`Пример:Номер мобильного телефона, привязанный к электронному кошельку получателя перевода. |
| {     "type": "add_info",     "title": "receiver_name",     "data": "Mark Doe" } | `Mark Doe`Пример:Полное имя получателя перевода. |
| {     "type": "add_info",     "title": "bank_name",     "data": "ORANGE" } | `ORANGE`Пример:Название банка получателя перевода. |
...
| --- | --- |
| Формат тела запроса | JSON |
| Конечная точка | `/v2/payment/wallet/orange/payout` |
| Полная спецификация конечной точки |  |
...
## Orange -Оплата 2: с отображением реквизитов
...
| **Возврат** |  |
| **Организация и стоимость подключения** | По согласованию с курирующим менеджером Some Platform |
| **Особенности**Если пользователь при оплате укажет сумму, отличную от суммы, указанной в запросе на оплату, то платеж будет отклонен.Платеж отклоняется, если пользователь не совершит оплату в течение 30 минут с момента инициирования платежа. |  |

---

## Orange -Оплата 1: перенаправление в сервис провайдера
...
| **Возврат** |  |
| **Организация и стоимость подключения** | По согласованию с курирующим менеджером Some Platform |
| **Особенности**Если пользователь при оплате укажет сумму, отличную от суммы, указанной в запросе на оплату, то платеж будет отклонен.Платеж отклоняется, если пользователь не совершит оплату в течение 30 минут с момента его перенаправления в сервис provider_service. |  |
