...
Получив и обработав запрос на оплату, платежная платформа направит вам оповещение (callback) с QR-кодом для отображения пользователю.

display_dataЧтобы отобразить пользователю QR-код, используйте информацию из массива.

display_dataЭлемент массивас QR-кодом содержит следующие параметры:

display_dataОстальные элементы массивасодержат следующие параметры:
...
"display_data": [
    {
        "type": "qr_img",
        "title": "QR code",
        "data": "data:image/png;base64,iVBCYII="
    },
    {
        "type": "add_info",
...
| Элемент | Описание |
| --- | --- |
| {     "type": "qr_img",     "title": "QR code",     "data": "data:image/png;base64,iVBCYII=" } | `data:image/png;base64,iVBCYII=`Пример:Данные, которые нужно отобразить в виде QR-кода. |
| {     "type": "add_info",     "title": "payment_amount_show",     "data": "1.05" } | `1.05`Пример:Сумма платежа в основных единицах валюты. |
| {     "type": "add_info",     "title": "operation_id",     "data": "12345678901234" } | `12345678901234`Пример:Номер платежа на стороне провайдера. |
| {     "type": "add_info",     "title": "currency_show",     "data": "USD" } | `USD`Пример:Валюта платежа. |
| {     "type": "add_info",     "title": "deeplink",     "data": "https://example.app/pay" } | `https://example.app/pay`Пример:Ссылка для перехода в мобильное приложение провайдера для оплаты. |
...
| title — это популярный в Эквадоре платежный метод, с помощью которого пользователи могут оплачивать в вашей системе различные услуги.Платежный метод title позволяет принимать от пользователей оплату, используя Payment Widget и Gate.здесьЗагрузить логотип этого платежного метода в векторном формате можно. |  |
| --- | --- |
