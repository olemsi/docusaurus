...
| phonetype_string type_strictly_required | `12345678901`Пример:Номер телефона пользователя. |  |
...
| addresstype_string type_strictly_required | `20 Baltimore St.`Пример:Адрес пользователя (улица, номер дома, номер корпуса, номер квартиры и т. п.). Должен содержать только буквы, цифры, знаки пробела. |  |
...
| by_methodtype_string type_strictly_required | `paysafecash``paysafecash`Пример:Код платежного метода (его значение всегда). |  |
...
| customer_addresstype_string type_strictly_required | `20 Baltimore St.`Пример:Адрес пользователя (улица, номер дома, номер корпуса, номер квартиры и т. п.). Должен содержать только буквы, цифры, знаки пробела. |
...
| **Регионы использования** | country_AT, country_ES, country_HR, country_HU, country_IT, country_MT, country_PT, country_RO, country_SI |
| --- | --- |
