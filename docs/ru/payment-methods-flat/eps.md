...
| Конечная точка | `/v2/payment/bank-transfer/eps/sale` |
...
| **Особенности**Метод доступен только для IP адресов из Австрии. Если пользователь будет использовать IP адрес другой страны, то ему отобразится страница с другими доступными методами оплаты. |  |
...
| title –  метод, позволяющий проводить платежи в евро с использованием банковских счетов в Австрии.Платежный метод title позволяет принимать от пользователей оплату, используя Payment Widget и Gate.здесьЗагрузить логотип этого платежного метода в векторном формате можно. |  |
| --- | --- |
