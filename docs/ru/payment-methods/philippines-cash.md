...
| Конечная точка | `/v2/payment/cash/philippines/sale` |
...
| accounttype_object type_strictly_required | bank_idtype_integer type_strictly_required | `2681`Пример:Идентификатор партнерской точки приема платежей. |
...
| --- | --- |
| Формат тела запроса | JSON |
| Конечная точка | `/v2/payment/cash/philippines/payout` |
| Полная спецификация конечной точки |  |
...
| last_nametype_string type_strictly_required | `Doe`Пример:Фамилия пользователя. |  |
| emailtype_string type_strictly_required | `johndoe@example.com`Пример:Адрес электронной почты пользователя. |  |
| phonetype_string type_strictly_required | `6312345678`Пример:Номер телефона пользователя. |  |
| day_of_birthtype_string type_strictly_required | `29-12-1984`Пример:Дата рождения пользователя, в формате ДД-ММ-ГГГГ. |  |
| addresstype_string type_strictly_required | `123 Sesame street`Пример:Адрес проживания пользователя. |  |
| citytype_string type_strictly_required | `Pasig`Пример:Город проживания пользователя. |  |
| countrytype_string type_strictly_required | `PH`Пример:Cтрана проживания пользователя. |  |
| statetype_string type_strictly_required | `Metro Manila`Пример:Штат или регион проживания пользователя. |  |
| districttype_string type_strictly_required | `Udong`Пример:Район проживания пользователя. |  |
| citizenshiptype_string type_strictly_required | `Philippines`Пример:Cтрана гражданства пользователя. |  |
| accounttype_object type_strictly_required | bank_idtype_integer type_strictly_required | `1621`Пример:Идентификатор партнерской точки выдачи наличных. |
| paymenttype_object type_strictly_required | currencytype_string type_strictly_required | `example_currency`Пример:Код валюты платежа в формате ISO-4217 alpha-3. |
| amounttype_integer type_strictly_required | *за исключением*Сумма платежа в дробных единицах валюты без десятичного разделителя (точки или запятой) и пробеловслучаев, когда у валюты нет дробной части.*основных единицах валюты*ru_currency_units.ditaru_currency_codes.ditaЕсли у валюты нет дробных единиц (то есть количество разрядов дробных единиц равно нулю), то в этом параметре нужно указывать сумму в. Подробнее о разрядах дробных единиц у валют см.и.`example_amount`Пример: example_amount_in_majors example_currency передается как |  |
| descriptiontype_integer type_strictly_required | `Описание операции.`Пример:Описание или комментарий к операции. |  |
| API ReferenceПри необходимости добавьте в запрос необязательные параметры, указанные в спецификации                     Gate:. |  |  |
...
**Отображение точек приема платежей на странице оплаты**

Платежи в методе title осуществляются через определенные точки приема платежей. Поэтому помимо платежного метода, на странице оплаты пользователю предлагается выбрать точку приема платежей, через которую он будет совершать оплату.

Метод title позволяет вам определить, как отображать точки приема платежей на странице оплаты:

**banks_idИдентификаторы точек приема платежей для передачи в параметре**

banks_idru_pm_philippines_cash_purchase_redirect_banks.ditaИдентификаторы точек приема платежей, которые вы можете передавать в параметре, чтобы отобразить пользователю на странице оплаты значки конкретных точек приема платежей, представлены в разделе.
...
- **Выбор точки приема платежей в два шага**Этот вариант используется по умолчанию. Пользователь выбирает точку приема платежей в два шага: сначала он выбирает title среди прочих методов, а затем — конкретную точку приема платежей из списка точек приема платежей, доступных в методе.
- **Отображение точек приема платежей на одной странице с платежными методами**split_bankspayment_methods_options`true`payment_methods_optionsВот пример использованияпри открытии страницы оплаты по ссылке:payment_methods_options={"pm_options_1": {"split_banks": true}}Чтобы отобразить точки приема платежей на странице оплаты в виде отдельных кнопок наряду с другими платежными методами, передайте параметрсо значениемв объекте.
- **Отображение точек приема платежей только одного метода**`force_pm`force_payment_methodЧтобы отобразить на странице оплаты только точки приема платежей (без других платежных методов), доступные в методе title, передайте в запросе на открытие страницы оплаты код платежного методав параметре.
- **Отображение только конкретных точек приема платежей**`force_pm`force_payment_methodbanks_idpayment_methods_optionsВот пример использованияпри открытии страницы оплаты по ссылке:`EPayWidget`Вот пример данных из запроса на открытие страницы оплаты с предварительно выбранными точками приема платежей через виджет.payment_methods_options={"pm_options_1": {"split_banks": true, "banks_id": [140, 141]}}Пример данных из запроса на оплату с несколькими предварительно выбранными точками приема платежей**payment_methods_options: '{"pm_options_1": {"banks_id": [140, 141]}}',**EPayWidget.run(
	{
		project_id: 1234,
...
)Чтобы отобразить на странице оплаты только определенные точки приема платежей из тех, что доступны в методе title, передайте в запросе на открытие страницы оплаты код платежного методав параметре, а также идентификаторы нужных точек приема платежей в массиве.
- **Принудительный выбор точки приема платежей**`force_pm`force_payment_methodbanks_idpayment_methods_optionsВот пример использованияпри открытии страницы оплаты по ссылке:`EPayWidget`Далее приведен пример данных из запроса на открытие страницы оплаты с предварительно выбранной точкой приема платежей через виджет.payment_methods_options={"pm_options_1": {"split_banks": true, "banks_id": [140]}}Пример данных из запроса на оплату с предварительно выбранной точкой приема платежей**payment_methods_options: '{"pm_options_1": {"banks_id": [140]}}',**EPayWidget.run(
	{
		project_id: 1234,
		payment_id: 'payment_47',
		customer_id: 'customer_123',
...
)Если в запросе на открытие страницы оплаты передать код платежного методав параметре, а также идентификатор одной из точек приема платежей в массиве, то страницы выбора метода и точек приема платежей будут пропущены, а пользователь будет сразу перенаправлен в сервис точки приема платежей.

---

## Philippines Payment Centers -Оплата
...
### Поддержка со стороны партнерских точек приема платежей

#### Список партнерских точек приема платежей

Проведение оплат с применением метода title осуществляется через партнерские точки приема платежей, поддерживающие работу с этим методом. Точкам соответствуют свои идентификаторы, которые используются при инициировании оплат через Gate, а также при вызове Payment Widget для отображения страницы с выбором метода оплаты.

В таблице ниже представлена информация о доступных для оплаты в партнерских точек приема платежей в методе title.

Данная таблица со списком партнерских точек приема платежей представлена в ознакомительных целях. Поскольку состав этого списка может меняться, рекомендуем уточнять актуальный список партнерских точек приема платежей, доступных при работе с методом title, с помощью запроса списка партнерских точек приема платежей.

| Точки приема платежей | Идентификатор |
| --- | --- |
| 7-Eleven | 2591 |
| Bayad Center | 2601 |
...
#### Запрос списка партнерских точек приема платежей

Далее представлена информация, необходимая для создания и отправки запроса на получение актуального списка партнерских точек приема платежей, поддерживаемых в методе title.

Вот пример данных из запроса списка партнерских точек приема платежей, поддерживающих работу с методом title.

| **HTTP-метод запроса** | POST |
| --- | --- |
| **Формат тела запроса** | JSON |
| **Конечная точка** | `/v2/info/banks/cash/philippines/sale/list` |
| **Полная спецификация конечной точки** |  |
...
### Список точек выдачи наличных

Проведение выплат с применением метода title осуществляется через сервисы точек выдачи наличных, поддерживающие работу с этим методом. Каждому сервису соответствуют свой идентификатор, который используется при инициировании выплат через Gate.

В таблице ниже представлена информация о доступных для выплаты точках выдачи наличных в методе title.

Данная таблица со списком точек выдачи наличных представлена в ознакомительных целях. Поскольку состав этого списка может меняться, рекомендуем уточнять актуальный список точек выдачи наличных, доступных при работе с методом title, с помощью запроса списка точек выдачи наличных.

| Точки выдачи наличных | Идентификатор |
| --- | --- |
...
### Запрос списка сервисов точек выдачи наличных

Далее представлена информация, необходимая для создания и отправки запроса на получение актуального списка сервисов точек выдачи наличных, поддерживаемых в методе title.

Вот пример данных из запроса списка сервисов точек выдачи наличных, поддерживающих работу с методом title.

| **HTTP-метод запроса** | POST |
| --- | --- |
| **Формат тела запроса** | JSON |
| **Конечная точка** | `/v2/info/banks/cash/philippines/payout/list` |
| **Полная спецификация конечной точки** |  |
...
## Philippines Payment Centers -Выплата
