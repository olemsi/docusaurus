...
To process a purchase through Gate by using one of the Open Banking in the UK method, the merchant's web service is required to do the following:
...
Information about the formats of requests and callbacks used for processing payments by using the Open Banking in the UK method via Gate is presented further in this section. General information about working with the Gate API is presented in .
...
There are several things you need to consider when sending purchase requests by using the Open Banking in the UK method:

Thus, a correct purchase request by using the Open Banking in the UK method must contain the project identifier, basic payment information (identifier, amount, and currency code), customer identifier and IP address, bank identifier, URL for redirection and signature, as well as various optional parameters.

1. `/v2/payment/banks/uk/sale`To initiate each purchase, send a separate POST request to theendpoint, which belongs to the group.
