...
| Конечная точка | `/v2/payment/online-banking/ideal/sale` |
...
| **Особенности**Метод доступен только для IP адресов из Нидерландов. Если пользователь будет использовать IP адрес другой страны, то ему отобразится страница с другими доступными методами оплаты. |  |
...
| title – метод, позволяющий проводить платежи в евро с использованием банковских счетов в Нидерландах.Платежный метод title позволяет принимать от пользователей оплату, используя Payment Widget и Gate.здесьЗагрузить логотип этого платежного метода в векторном формате можно. |  |
| --- | --- |
