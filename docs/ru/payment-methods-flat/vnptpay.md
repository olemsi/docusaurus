...
| Конечная точка | `/v2/payment/wallet/vnptpay/sale` |
...
| amounttype_integer type_strictly_required | Сумма платежа в основных единицах валюты, без дробной части.*в дробных единицах валюты без десятичного разделителя (точки или запятой) и пробелов*ru_currency_codes.ditaЕсли у валюты есть дробные единицы (то есть количество разрядов дробных единиц не равно нулю), то в этом параметре нужно указывать сумму в. Подробнее о разрядах дробных единиц у валют см..`example_amount`Пример: example_amount_in_majors example_currency передается как |  |
...
| payment_amounttype_integer type_strictly_required | Сумма платежа в основных единицах валюты, без дробной части.*в дробных единицах валюты без десятичного разделителя (точки или запятой) и пробелов*ru_currency_codes.ditaЕсли у валюты есть дробные единицы (то есть количество разрядов дробных единиц не равно нулю), то в этом параметре нужно указывать сумму в. Подробнее о разрядах дробных единиц у валют см..`example_amount`Пример: example_amount_in_majors example_currency передается как |
