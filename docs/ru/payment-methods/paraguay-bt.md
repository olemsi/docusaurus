...
|  | Интерфейсы | references/ru/currencies/PYG.ditaСуммы платежа, | Время проведения платежа |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Payment Widget | Gate | Dashboard | минимум | максимум | базовое | предельное |  |
| Оплата |  |  |  | 10&#160;000 | 70&#160;000&#160;000 | 10&#160;минут | 3&#160;часа |
| Выплата |  |  |  | 10&#160;000 | 70&#160;000&#160;000 | 10&#160;минут | 48&#160;часов |

| title — это популярный в Парагвае платежный метод, с помощью которого пользователи могут оплачивать в вашей системе различные услуги, а также получать денежные средства. В платежном методе title оплата совершается с использованием банковских переводов.Платежный метод title позволяет принимать от пользователей оплату, используя Payment Widget и Gate, а также проводить пользователям выплату через Gate и Dashboard.здесьЗагрузить логотип этого платежного метода в векторном формате можно. |  |
| --- | --- |

| Тип платежного метода | Банковские переводы |
| --- | --- |
| Регионы использования | country_PY |
| Валюты платежей | references/ru/currencies/PYG.dita |
...
| Частичный возврат |  |
| Опротестование |  |
| Особенности | ru_gate_amount_modification.ditaЕсли пользователь выполнит перевод на сумму, отличную от суммы, указанной в запросе на оплату, то платеж будет проводиться с особенностями, которые описаны в разделе.references/ru/currencies/PYG.ditareferences/ru/currencies/PYG.dita`10000`Например, при оплате на 10&#160;000в запросе нужно передавать значение.ru_currency_codes.ditaЧисло дробных единиц для различных валют указано в разделе.Суммы в запросах с использованием валютыуказываются в основных единицах валюты в соответствии со стандартом ISO-4217 alpha-3.Если пользователь не выполнит необходимые для оплаты действия в течение 3 часов, то платеж будет отклонен.В некоторых случаях выплаты могут проводиться пользователям только в определенное время.В рабочее время: выплаты по запросам, поступившим до 17:00 (по парагвайскому времени), зачисляются пользователям в 17:00, а оповещение о выплате будет отправлено вашей системе в 23:59.В нерабочее время и выходные: выплаты по запросам, поступившим в выходной день или после 17:00 (по парагвайскому времени) в рабочий день, зачисляются пользователям в 17:00 следующего рабочего дня. Оповещение о выплате будет отправлено вашей системе также на следующий рабочий день в 23:59.Уточняйте у своего курирующего менеджера Some Platform, актуальна ли эта особенность для вас.Указанные в инструкции реквизиты могут использоваться для оплаты только один раз. Для повторной оплаты пользователь должен инициировать новый платеж. |
...
| Конечная точка | `/v2/payment/bank-transfer/paraguay/sale` |
...
| identifytype_object type_optional | doc_numbertype_string type_optional | `12345678`Пример:Номер CI (Cédula de identidad, удостоверение личности) пользователя, состоит из 6-8 цифр. |
...
| --- | --- |
| Формат тела запроса | JSON |
| Конечная точка | `/v2/payment/bank-transfer/paraguay/payout` |
| Полная спецификация конечной точки |  |
...
| last_nametype_string type_strictly_required | `Doe`Пример:Фамилия пользователя. |  |
| emailtype_string type_strictly_required | `johndoe@example.com`Пример:Адрес электронной почты пользователя. |  |
| identifytype_object type_strictly_required | doc_numbertype_string type_strictly_required | `12345678`Пример:Номер CI (Cédula de identidad, удостоверение личности) пользователя, состоит из 6-8 цифр. |
| doc_typetype_string type_strictly_required | `CI``CI`Пример:Тип документа, подтверждающего личность пользователя —(Cédula de identidad, удостоверение личности). |  |
...
| bank_idtype_integer type_strictly_required | ru_pm_paraguay_bt_payout_banks.dita`76516`Пример:Идентификатор банка пользователя. Подробнее о том, как получить список поддерживаемых банков см.. |  |
| paymenttype_object type_strictly_required | currencytype_string type_strictly_required | `example_currency`Пример:Код валюты платежа в формате ISO-4217 alpha-3. |
| amounttype_integer type_strictly_required | Сумма платежа в основных единицах валюты, без дробной части.*в дробных единицах валюты без десятичного разделителя (точки или запятой) и пробелов*ru_currency_units.ditaru_currency_codes.ditaЕсли у валюты есть дробные единицы (то есть количество разрядов дробных единиц не равно нулю), то в этом параметре нужно указывать сумму в. Подробнее о разрядах дробных единиц у валют см.и.`example_amount`Пример: example_amount_in_majors example_currency передается как |  |
| API ReferenceПри необходимости добавьте в запрос необязательные параметры, указанные в спецификации                     Gate:. |  |  |
...
| --- | --- |
| **Формат тела запроса** | JSON |
| **Конечная точка** | `/v2/info/banks/{parent_method}/{child_method}/sale/list` |
| **Полная спецификация конечной точки** |  |
...
## Bank transfers in Paraguay -Выплата
...
| --- | --- |
| **Валюты выплаты** | references/ru/currencies/PYG.dita |
...
| **Возврат** |  |
| **Организация и стоимость подключения** | По согласованию с курирующим менеджером Some Platform |
| **Особенности**references/ru/currencies/PYG.ditareferences/ru/currencies/PYG.dita`1000`Например, при оплате на 1000в запросе нужно передавать значение.ru_currency_codes.ditaЧисло дробных единиц для различных валют указано в разделе.Суммы в запросах с использованием валютыуказываются в основных единицах валюты в соответствии со стандартом ISO-4217 alpha-3.В некоторых случаях выплаты могут проводиться пользователям только в определенное время. В рабочее время: выплаты по запросам, поступившим до 17:00 (по парагвайскому времени), зачисляются пользователям в 17:00, а оповещение о выплате будет отправлено вашей системе в 23:59. В нерабочее время и выходные: выплаты по запросам, поступившим в выходной день или после 17:00 (по парагвайскому времени) в рабочий день, зачисляются пользователям в 17:00 следующего рабочего дня. Оповещение о выплате будет отправлено вашей системе также на следующий рабочий день в 23:59. Уточняйте у своего курирующего менеджера Some Platform, актуальна ли эта особенность для вас. |  |
...
| identify_doc_numbertype_string type_optional | `12345678`Пример:Номер CI (Cédula de identidad, удостоверение личности) пользователя, состоит из 6-8 цифр. |
...
## Bank transfers in Paraguay -Оплата
...
| **Регионы использования** |  |
| --- | --- |
| **Валюты оплаты** | references/ru/currencies/PYG.dita |
| **Суммы платежей** | Информацию уточняйте у курирующего менеджера Some Platform. Также вы можете уточнить в Dashboard минимальную и максимальную сумму платежа, доступную в вашем проекте. |
| **Время проведения платежа** | Информацию уточняйте у курирующего менеджера Some Platform. |
| **Конвертация валют** | На стороне Some Platform |
| **Возврат** |  |
| **Организация и стоимость подключения** | По согласованию с курирующим менеджером Some Platform |
| **Особенности**references/ru/currencies/PYG.ditareferences/ru/currencies/PYG.dita`1000`Например, при оплате на 1000в запросе нужно передавать значение.ru_currency_codes.ditaЧисло дробных единиц для различных валют указано в разделе.Суммы в запросах с использованием валютыуказываются в основных единицах валюты в соответствии со стандартом ISO-4217 alpha-3.ru_gate_amount_modification.ditaЕсли пользователь при оплате укажет сумму, отличную от суммы, указанной в запросе на оплату, то платеж будет проводиться с особенностями, которые описаны в разделе.Реквизиты для оплаты должны использоваться только один раз. Для повторной оплаты пользователь должен инициировать новый платеж.Платеж отклоняется, если пользователь не совершит оплату в течение 3 часов с момента его перенаправления в сервис provider_service. |  |
