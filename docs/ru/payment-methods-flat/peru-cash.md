...
| Конечная точка | `/v2/payment/cash/peru/sale` |
...
| identifytype_object type_optional | doc_numbertype_string type_optional | doc_type8 цифр — для документа DNI;11 цифр — для RUC.`12345678901`Пример:Номер документа, подтверждающего личность пользователя. Номер документа должен соответствовать типу, передаваемому в параметреи содержать определенное количество символов: |
| doc_typetype_string type_optional | Возможные значения:`RUC`Пример:`DNI`— для удостоверения личности гражданина Перу (Documento Nacional de Identidad);`RUC`— для идентификатора налогоплательщика в Перу (Registro Único de Contribuyente).Тип документа, подтверждающего личность пользователя. |  |
...
| identify_doc_numbertype_string type_optional | doc_type8 цифр — для документа DNI;11 цифр — для RUC.`12345678901`Пример:Номер документа, подтверждающего личность пользователя. Номер документа должен соответствовать типу, передаваемому в параметреи содержать определенное количество символов: |
| identify_doc_typetype_string type_optional | Возможные значения:`RUC`Пример:`DNI`— для удостоверения личности гражданина Перу (Documento Nacional de Identidad);`RUC`— для идентификатора налогоплательщика в Перу (Registro Único de Contribuyente).Тип документа, подтверждающего личность пользователя. |
...
| --- | --- |
| **Валюты оплаты** | references/ru/currencies/PEN.ditareferences/ru/currencies/USD.dita, |
...
| **Особенности**Срок действия ваучера уточняйте у вашего курирующего менеджера в Some Platform. |  |
