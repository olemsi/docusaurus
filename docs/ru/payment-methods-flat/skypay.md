...
| by_methodtype_string type_strictly_required | `SkyPay``SkyPay`Пример:Код платежного метода (его значение всегда). |  |  |
...
| payment_datatype_object type_required | accounttype_object type_required | numbertype_string type_required | `12345678****1234`Пример:Маскированный номер карты пользователя в формате 12345678****1234. |
...
| **Регионы использования** | country_BY, country_KZ, country_RU, country_UA |
| --- | --- |
...
| **Конвертация валют** | На стороне сервиса provider_service |
| **Возврат** |  |
