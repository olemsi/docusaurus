...
| Конечная точка | `/v2/payment/wallet/satispay/sale` |
...
| first_nametype_string type_required | `John`Пример:Имя пользователя (для предотвращения ошибок при проведении платежей рекомендуется указывать не менее 3 и не более 100 символов). |  |
| last_nametype_string type_required | `Doe`Пример:Фамилия пользователя (для предотвращения ошибок при проведении платежей рекомендуется указывать не менее 3 и не более 100 символов). |  |
| emailtype_string type_strictly_required | `johndoe@example.com`Пример:Адрес электронной почты пользователя. |  |
| countrytype_string type_required | `IT`Пример:Код страны пользователя в формате ISO 3166-1 alpha-2. |  |
...
| --- | --- |
| Формат тела запроса | JSON |
| Конечная точка | `/v2/payment/wallet/satispay/refund` |
| Полная спецификация конечной точки |  |
...
| customer_first_nametype_string type_required | `John`Пример:Имя пользователя (для предотвращения ошибок при проведении платежей рекомендуется указывать не менее 3 и не более 100 символов). |
| customer_last_nametype_string type_required | `Doe`Пример:Фамилия пользователя (для предотвращения ошибок при проведении платежей рекомендуется указывать не менее 3 и не более 100 символов). |
...
| customer_countrytype_string type_required | `IT`Пример:Код страны пользователя в формате ISO 3166-1 alpha-2. |
...
| **Регионы использования** | country_AT, country_BE, country_BG, country_CH, country_CY, country_CZ, country_DE, country_DK, country_EE, country_ES, country_FI, country_FR, country_GB, country_GR, country_HR, country_HU, country_IE, country_IS, country_IT, country_LI, country_LT, country_LU, country_LV, country_MT, country_NL, country_NO, country_PL, country_PT, country_RO, country_SE, country_SI, country_SK |
| --- | --- |
