...
| Конечная точка | `/v2/payment/voucher/pagoefectivo/sale` |
...
| identifytype_object type_strictly_required | doc_numbertype_string type_strictly_required | `12345678`Пример:Номер DNI (Documento Nacional de Identidad) пользователя. Должен содержать от 8 до 9 цифр, без специальных символов и знаков пунктуации. |
...
| identify_doc_numbertype_string type_required | `12345678`Пример:Номер DNI (Documento Nacional de Identidad) пользователя. Должен содержать от 8 до 9 цифр, без специальных символов и знаков пунктуации. |
...
| **Особенности**Срок действия ваучера по умолчанию составляет 5 дней и может быть изменен. За подробной информацией обращайтесь к специалистам технической поддержки Some Platform. |  |
