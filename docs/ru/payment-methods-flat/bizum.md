...
| Конечная точка | `/v2/payment/bank-transfer/bizum/sale` |
...
## Bizum: Bank transfers in Spain -Оплата
...
| **Особенности**Платеж отклоняется, если пользователь не совершит оплату в течение 4 минут с момента его перенаправления в сервис provider_service.references/ru/currencies/EUR.ditareferences/ru/currencies/EUR.ditaru_currency_codes.ditaСумму в запросе указывайте в дробных единицах валюты! Например, сумма 95,00в запросе представляется как 9500. Число дробных единиц для различных валют можно узнать в разделе.Сумма платежа в валютедолжна быть целочисленной, иначе платеж будет отклонен. |  |
...
| title – один из самых распространенных методов оплаты в Испании. Это быстрый способ оплаты с высоким коэффициентом конверсии. Для проведения платежей пользователю необходимо авторизоваться, введя номер мобильного телефона в сервисе Bizum.Платежный метод title позволяет принимать от пользователей оплату, используя Payment Widget и Gate.здесьЗагрузить логотип этого платежного метода в векторном формате можно. |  |
| --- | --- |
