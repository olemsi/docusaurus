...
| Конечная точка | `/v2/payment/wallet/instapay/sale` |
...
| {     "type": "add_info",     "title": "receiver_account",     "data": "ad72" } | `ad72`Пример:Номер электронного кошелька, на который пользователь совершает перевод. |

| Элемент | Описание |
| --- | --- |
| {     "type": "qr_data",     "title": "QR code",     "data": "https://example.com" } | `https://example.com`Пример:Ссылка, по которой отображается QR-код. |
| {     "type": "add_info",     "title": "lifetime",     "data": "480" } | `480`Пример:Срок действия реквизитов в секундах. |
...
| --- | --- |
| Формат тела запроса | JSON |
| Конечная точка | `/v2/payment/wallet/instapay/payout` |
| Полная спецификация конечной точки |  |
...
| last_nametype_string type_strictly_required | `Doe`Пример:Фамилия пользователя. |  |
| emailtype_string type_strictly_required | `johndoe@example.com`Пример:Адрес электронной почты пользователя. |  |
| accounttype_object type_strictly_required | numbertype_string type_strictly_required | `user123`Пример:Идентификатор электронного кошелька пользователя. |
| paymenttype_object type_strictly_required | currencytype_string type_strictly_required | `example_currency`Пример:Код валюты платежа в формате ISO-4217 alpha-3. |
| amounttype_integer type_strictly_required | *за исключением*Сумма платежа в дробных единицах валюты без десятичного разделителя (точки или запятой) и пробеловслучаев, когда у валюты нет дробной части.*основных единицах валюты*ru_currency_units.ditaru_currency_codes.ditaЕсли у валюты нет дробных единиц (то есть количество разрядов дробных единиц равно нулю), то в этом параметре нужно указывать сумму в. Подробнее о разрядах дробных единиц у валют см.и.`example_amount`Пример: example_amount_in_majors example_currency передается как |  |
| API ReferenceПри необходимости добавьте в запрос необязательные параметры, указанные в спецификации                     Gate:. |  |  |
...
| **Особенности**references/ru/currencies/EGP.ditareferences/ru/currencies/EGP.ditareferences/ru/currencies/EGP.ditareferences/ru/currencies/EGP.ditaСумма платежа в валютедолжна быть целочисленной (т. е. значение после запятой должно равняться нулю, например 110,00, 215,00, 323,00и т. д.), иначе платеж будет отклонен. |  |
...
## InstaPay Wallet -Оплата 2: с отображением реквизитов
...
| **Возврат** |  |
| **Организация и стоимость подключения** | По согласованию с курирующим менеджером Some Platform |
| **Особенности**ru_gate_amount_modification.ditaЕсли пользователь при оплате укажет сумму, отличную от суммы, указанной в запросе на оплату, то платеж будет проводиться с особенностями, которые описаны в разделе.Реквизиты для оплаты должны использоваться только один раз. Для повторной оплаты пользователь должен инициировать новый платеж. |  |

---

## InstaPay Wallet -Оплата 1: перенаправление в сервис провайдера
...
| **Возврат** |  |
| **Организация и стоимость подключения** | По согласованию с курирующим менеджером Some Platform |
| **Особенности**references/ru/currencies/EGP.ditareferences/ru/currencies/EGP.ditareferences/ru/currencies/EGP.ditareferences/ru/currencies/EGP.ditaСумма платежа в валютедолжна быть целочисленной (т. е. значение после запятой должно равняться нулю, например 110,00, 215,00, 323,00и т. д.), иначе платеж будет отклонен.Если пользователь при оплате укажет сумму, отличную от суммы, указанной в запросе на оплату, то платеж будет отклонен.Реквизиты для оплаты должны использоваться только один раз. Для повторной оплаты пользователь должен инициировать новый платеж.Если пользователь совершит оплату после истечения времени, платеж не будет зачислен. Для оплаты пользователь должен инициировать новый платеж.Платеж отклоняется, если пользователь не совершит оплату в течение 8 минут с момента его перенаправления в сервис provider_service. |  |
