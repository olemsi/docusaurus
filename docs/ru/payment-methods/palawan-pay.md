...
| Конечная точка | `/v2/payment/wallet/palawanpay/sale` |
...
| --- | --- |
| Формат тела запроса | JSON |
| Конечная точка | `/v2/payment/wallet/palawanpay/payout` |
| Полная спецификация конечной точки |  |
...
account.numberВ этом методе в оповещении дополнительно передаётся параметрс номером электронного кошелька пользователя.
