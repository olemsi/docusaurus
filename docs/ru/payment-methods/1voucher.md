...
| Конечная точка | `/v2/payment/voucher/1voucher/sale` |
...
provider_extra_fieldsamount_remnantpinВ этом методе в оповещении дополнительно передается объект, содержащий параметры:с оставшейся суммой на балансе ваучера и— с PIN-кодом ваучера. Эти данные передаются, так как пользователь может использовать не весь ваучер за один платеж.
...
1. Отправьте запрос с нужными параметрами и подписью на рабочий URL-адрес платежной платформы Some Platform.
2. Примите от платежной платформы Some Platform оповещение (callback) с результатом выплаты и PIN-кодом ваучера, который вы должны отобразить пользователю.
...
| --- | --- |
| Формат тела запроса | JSON |
| Конечная точка | `/v2/payment/voucher/1voucher/payout` |
| Полная спецификация конечной точки |  |
...
provider_extra_fieldsamount_remnantpinВ этом методе в оповещении дополнительно передается объект, содержащий параметры:с суммой на балансе ваучера и— с PIN-кодом ваучера. Эти данные пользователь должен использовать для получения выплаты в любом месте, где принимается title — как онлайн, так и в супермаркетах.
...
При выплате пользователь инициирует платеж, после чего получает PIN-код ваучера. Пользователь должен использовать ваучер для получения выплаты в любом месте, где принимается title — как онлайн, так и в супермаркетах.
...
| --- | --- |
| **Валюты выплаты** | references/ru/currencies/ZAR.dita |
...
| **Регионы использования** |  |
| --- | --- |
| **Валюты оплаты** | references/ru/currencies/ZAR.dita |
| **Суммы платежей** | Информацию уточняйте у курирующего менеджера Some Platform. Также вы можете уточнить в Dashboard минимальную и максимальную сумму платежа, доступную в вашем проекте. |
| **Время проведения платежа** | Информацию уточняйте у курирующего менеджера Some Platform. |
| **Конвертация валют** | На стороне Some Platform |
| **Возврат** |  |
...
| --- | --- |
| Регионы использования |  |
| Валюты платежей | references/ru/currencies/ZAR.dita |
| Суммы платежей | Информацию уточняйте у курирующего менеджера Some Platform. Также вы можете уточнить в Dashboard минимальную и максимальную сумму платежа, доступную в вашем проекте. |
| Время проведения платежа | Информацию уточняйте у курирующего менеджера Some Platform. |
| Конвертация валют | На стороне Some Platform |
| Оплата |  |
