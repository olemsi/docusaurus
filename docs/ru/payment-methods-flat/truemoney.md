...
| Конечная точка | `/v2/payment/wallet/true-money/sale` |
...
ru_pm_truemoney_gate_purchase_redirect.ditaПодробная информация о проведении оплаты с использованием метода title через Gate представлена в разделе.
...
ru_pm_truemoney_pp_purchase_redirect.ditaПодробная информация о проведении оплаты с использованием метода title через Payment Widget представлена в разделе.
...
| **Особенности**Срок действия QR-кода на стороне провайдера — 2 минуты или 5 минут (зависит от провайдера, проводящего платеж).Реквизиты для оплаты должны использоваться только один раз. Для повторной оплаты пользователь должен инициировать новый платеж. |  |
...
| Конвертация валют | На стороне Some Platform |
| Оплата |  |
| Выплата | ru_pm_thailand_banks.ditaПроводится с использованием метода. |
| Повторяемая оплата |  |
| Возврат |  |
| Организация и стоимость подключения | По согласованию с курирующим менеджером Some Platform |
