...
| paymenttype_object type_strictly_required | currencytype_string type_strictly_required | references/ru/currencies/EUR.ditareferences/ru/currencies/UAH.ditareferences/ru/currencies/USD.dita`example_currency`Пример:Код валюты платежа в формате ISO-4217 alpha-3. В качестве валюты оплаты можно использовать только,,. |
...
account.numberВ этом методе в оповещении дополнительно передаётся параметрс адресом электронной почты пользователя, к которому привязан счет кошелька.
...
| last_nametype_string type_strictly_required | `Doe`Пример:Фамилия пользователя. |  |
| emailtype_string type_strictly_required | `johndoe@example.com`Пример:Адрес электронной почты пользователя. |  |
| accounttype_object type_strictly_required | numbertype_string type_strictly_required | `johndoe@example.com`Пример:Адрес электронной почты пользователя, к которому привязан счет кошелька. |
| paymenttype_object type_strictly_required | currencytype_string type_strictly_required | references/ru/currencies/EUR.ditareferences/ru/currencies/UAH.ditareferences/ru/currencies/USD.dita`example_currency`Пример:Код валюты платежа в формате ISO-4217 alpha-3. В качестве валюты выплаты можно использовать только,,. |
| amounttype_integer type_strictly_required | *за исключением*Сумма платежа в дробных единицах валюты без десятичного разделителя (точки или запятой) и пробеловслучаев, когда у валюты нет дробной части.*основных единицах валюты*ru_currency_units.ditaru_currency_codes.ditaЕсли у валюты нет дробных единиц (то есть количество разрядов дробных единиц равно нулю), то в этом параметре нужно указывать сумму в. Подробнее о разрядах дробных единиц у валют см.и.`example_amount`Пример: example_amount_in_majors example_currency передается как |  |
| API ReferenceПри необходимости добавьте в запрос необязательные параметры, указанные в спецификации                     Gate:. |  |  |
...
| **Регионы использования** | country_KZ, country_RU, country_UA |
| --- | --- |
| **Валюты выплаты** | references/ru/currencies/EUR.ditareferences/ru/currencies/UAH.ditareferences/ru/currencies/USD.dita,, |
...
| **Особенности**references/ru/currencies/EUR.ditareferences/ru/currencies/UAH.ditareferences/ru/currencies/USD.ditaВ качестве валюты оплаты можно использовать только,,. |  |
...
| payment_currencytype_string type_strictly_required | references/ru/currencies/EUR.ditareferences/ru/currencies/UAH.ditareferences/ru/currencies/USD.dita`example_currency`Пример:Код валюты платежа в формате ISO-4217 alpha-3. В качестве валюты оплаты можно использовать только,,. |
...
| **Возврат** |  |
| **Организация и стоимость подключения** | По согласованию с курирующим менеджером Some Platform |
| **Особенности**references/ru/currencies/EUR.ditareferences/ru/currencies/UAH.ditareferences/ru/currencies/USD.ditaВ качестве валюты оплаты можно использовать только,,.Возможно отображение выбора валюты платежа на Payment Widget. |  |
