...
| Конечная точка | `v2/payment/bank-transfer/hk/sale` |
...
| returntype_string type_strictly_required | `https://example.com/return/`Пример:URL-адрес, куда следует перенаправить пользователя в случае прерывания оплаты пользователем до ее завершения. |  |
...
## Bank transfers in Hong Kong -Оплата
