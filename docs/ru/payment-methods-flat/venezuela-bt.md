...
| Конечная точка | `/v2/payment/bank-transfer/venezuela/sale` |
...
| --- | --- |
| Формат тела запроса | JSON |
| Конечная точка | `/v2/payment/bank-transfer/venezuela/payout` |
| Полная спецификация конечной точки |  |
...
| last_nametype_string type_strictly_required | `Doe`Пример:Фамилия пользователя. |  |
| emailtype_string type_strictly_required | `johndoe@example.com`Пример:Адрес электронной почты пользователя. |  |
| identifytype_strictly_required✱type_object | doc_numbertype_strictly_required✱type_string | `VE``V12345678`Пример:Номер удостоверения личности (Documento de identidad) пользователя. Должен начинаться с одной из букв:илии содержать восемь цифр. |
| accounttype_object type_strictly_required | numbertype_string type_strictly_required | `12345678901`Пример:Номер телефона, к которому привязан счет (состоит из 11 цифр, без указания кода страны), или номер счета пользователя. |
| typetype_strictly_required✱type_string | Возможные значения:`CAH`Пример:`CAH`— для сберегательного счета (Cuenta ahorro);`CCR`— для текущего счета (Cuenta corriente);Тип счета пользователя. |  |
| bank_idtype_integer type_strictly_required | ru_pm_venezuela_bt_payout_banks.dita`74522`Пример:Идентификатор банка пользователя. Подробнее о том, как получить список поддерживаемых банков см.. |  |
| paymenttype_object type_strictly_required | currencytype_string type_strictly_required | `example_currency`Пример:Код валюты платежа в формате ISO-4217 alpha-3. |
| amounttype_integer type_strictly_required | *за исключением*Сумма платежа в дробных единицах валюты без десятичного разделителя (точки или запятой) и пробеловслучаев, когда у валюты нет дробной части.*основных единицах валюты*ru_currency_units.ditaru_currency_codes.ditaЕсли у валюты нет дробных единиц (то есть количество разрядов дробных единиц равно нулю), то в этом параметре нужно указывать сумму в. Подробнее о разрядах дробных единиц у валют см.и.`example_amount`Пример: example_amount_in_majors example_currency передается как |  |
| API ReferenceПри необходимости добавьте в запрос необязательные параметры, указанные в спецификации                     Gate:. |  |  |
...
| BANCAMIGA | 74652 |
| BANCO ACTIVO | 74642 |
| Banco Agrícola de Venezuela C.A. | 83982 |
| Banco Caroni | 79222 |
| Banco de la Fuerza Armada Nacional Bolivariana | 83992 |
| Banco de la Gente Emprendedora C.A. | 83962 |
| Banco de Venezuela | 79212 |
| BANCO DEL CARIBE | 74562 |
| BANCO DEL TESORO | 74612 |
| Banco Digital de los Trabajadores (BICENTENARIO) | 74672 |
| BANCO EXTERIOR | 74572 |
| Banco Fondo Común, C.A Banco Universal | 83872 |
| Banco Internacional de Desarrollo | 79242 |
...
| BANCRECER | 74622 |
| Banesco | 72432 |
| Banplus Banco Universal | 79252 |
| DelSur, Banco Universal C.A. | 83972 |
| MIBANCO | 74632 |
| N58 Banco Digital, Banco Microfinanciero | 84002 |
...
| --- | --- |
| **Формат тела запроса** | JSON |
| **Конечная точка** | `/v2/info/banks/bank-transfer/venezuela/payout/list` |
| **Полная спецификация конечной точки** |  |
...
## Bank transfers in Venezuela -Выплата
...
| --- | --- |
| **Валюты выплаты** | references/ru/currencies/VES.dita |
...
## Bank transfers in Venezuela -Оплата
...
| **Регионы использования** |  |
| --- | --- |
| **Валюты оплаты** | references/ru/currencies/VES.dita |
| **Суммы платежей** | Информацию уточняйте у курирующего менеджера Some Platform. Также вы можете уточнить в Dashboard минимальную и максимальную сумму платежа, доступную в вашем проекте. |
| **Время проведения платежа** | Информацию уточняйте у курирующего менеджера Some Platform. |
| **Конвертация валют** | На стороне Some Platform |
| **Возврат** |  |
| **Организация и стоимость подключения** | По согласованию с курирующим менеджером Some Platform |
| **Особенности**Если пользователь при оплате укажет сумму, отличную от суммы, указанной в запросе на оплату, то платеж будет отклонен.Реквизиты для оплаты должны использоваться только один раз. Для повторной оплаты пользователь должен инициировать новый платеж.Платеж отклоняется, если пользователь не совершит оплату в течение 5 минут с момента инициирования платежа.references/ru/currencies/VES.ditareferences/ru/currencies/VES.ditaru_currency_codes.ditaСумму в запросе указывайте в дробных единицах валюты! Например, сумма 95,00в запросе представляется как 9500. Число дробных единиц для различных валют можно узнать в разделе.Сумма платежа в валютедолжна быть целочисленной, иначе платеж будет отклонен. |  |
...
| --- | --- |
| Регионы использования |  |
| Валюты платежей | references/ru/currencies/VES.dita |
| Суммы платежей | Информацию уточняйте у курирующего менеджера Some Platform. Также вы можете уточнить в Dashboard минимальную и максимальную сумму платежа, доступную в вашем проекте. |
| Время проведения платежа | Информацию уточняйте у курирующего менеджера Some Platform. |
| Конвертация валют | На стороне Some Platform |
| Оплата |  |
