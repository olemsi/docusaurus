...
| Конечная точка | `/v2/payment/wallet/tigomoney/sale` |
...
| identifytype_object type_optional | doc_numbertype_string type_optional | `1234567`Пример:Номер документа, подтверждающего личность пользователя. |
...
| account_numbertype_string type_optional | `1234567`Пример:Номер счета пользователя. |
...
| **Особенности**Для проведения платежей пользователю потребуется зарегистрироваться в приложении Tigo Money.Платеж отклоняется, если пользователь не совершит оплату в течение 2 минут. |  |
