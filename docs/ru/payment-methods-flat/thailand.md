...
| Конечная точка | `/v2/payment/banks/thailand/sale` |
...
| languagetype_string type_strictly_required | customer.languageПо умолчанию платежная страница сервиса title открывается на английском языке. Для изменения языка платежной страницы необходимо передать параметрсо значением кода языка. Далее приведен список поддерживаемых языков с их кодами, но точный список языков рекомендуется уточнять у специалистов технической поддержки Some Platform (support@example.com английский,`id`— индонезийский,`ms`— малайский,`my`— бирманский,`th`— тайский,`vi`— вьетнамский,`zh`— китайский.Код языка платежной страницы. |  |
| accounttype_object type_strictly_required | bank_idtype_integer type_strictly_required | ru_pm_thailand_banks_purchase_redirect_banks.dita`4181`Пример:Идентификатор банка пользователя. Подробнее о том, как получить список поддерживаемых банков см.. |
...
ru_pm_thailand_banks_gate_purchase_redirect.ditaПодробная информация о проведении оплаты с использованием метода title через Gate представлена в разделе.

- **тестового**в запросах вы должны передавать идентификаторпроекта;
- `4181`в качестве идентификатора банка пользователя в запросе можно использовать только;
...
| --- | --- |
| Формат тела запроса | JSON |
| Конечная точка | `/v2/payment/banks/thailand/payout` |
| Полная спецификация конечной точки |  |
...
| bank_idtype_integer type_strictly_required | ru_pm_thailand_banks_payout_banks.dita`61021`Пример:Идентификатор банка пользователя. Подробнее о том, как получить список поддерживаемых банков см.. |  |
| customer_nametype_string type_strictly_required | `จอห์น โด`document_numberЕсли имя передается на тайском языке (пример:), то в параметренеобходимо указать настоящий TaxID пользователя.`John Doe`document_numberЕсли имя передается латиницей (пример:), то указывать TaxID внет необходимости.`จอห์น โด`Пример:Полное имя пользователя. |  |
| document_numbertype_string type_strictly_required | customer_nameПередавайте этот параметр, только еслисодержит имя пользователя на тайском языке.`1234567890123`Пример:Идентификационный номер налогоплательщика в Таиланде. |  |
| paymenttype_object type_strictly_required | currencytype_string type_strictly_required | `example_currency`Пример:Код валюты платежа в формате ISO-4217 alpha-3. |
| amounttype_integer type_strictly_required | *за исключением*Сумма платежа в дробных единицах валюты без десятичного разделителя (точки или запятой) и пробеловслучаев, когда у валюты нет дробной части.*основных единицах валюты*ru_currency_units.ditaru_currency_codes.ditaЕсли у валюты нет дробных единиц (то есть количество разрядов дробных единиц равно нулю), то в этом параметре нужно указывать сумму в. Подробнее о разрядах дробных единиц у валют см.и.`example_amount`Пример: example_amount_in_majors example_currency передается как |  |
| API ReferenceПри необходимости добавьте в запрос необязательные параметры, указанные в спецификации                     Gate:. |  |  |
...
ru_pm_thailand_banks_gate_payout.ditaПодробная информация о проведении выплаты с использованием метода title через Gate представлена в разделе.
...
| Mizuho Bank Bangkok Branch | 60071 |
| Mizuho Corporate Bank | 5891 |
| Siam Commercial Bank | 125 |
| Standard Chartered Bank | 4711 |
| Sumitomo Mitsui Banking Corporation | 5901 |
| Sumitomo Mitsui Trust Bank (Thai) Public Company Limited | 22601 |
| Thai Credit Retail Bank | 5941 |
| Tisco Bank | 66661 |
...
| --- | --- |
| **Формат тела запроса** | JSON |
| **Конечная точка** | `/v2/info/banks/thailand/payout/list` |
| **Полная спецификация конечной точки** |  |
...
| --- | --- |
| **Валюты выплаты** | references/ru/currencies/THB.dita |
...
banks_idru_pm_thailand_banks_purchase_redirect_banks.ditaИдентификаторы банков, которые вы можете передавать в параметре, чтобы отобразить пользователю на странице оплаты значки конкретных банков, представлены в разделе.
...
| language_codetype_string type_strictly_required | customer.languageПо умолчанию платежная страница сервиса title открывается на английском языке. Для изменения языка платежной страницы необходимо передать параметрсо значением кода языка. Далее приведен список поддерживаемых языков с их кодами, но точный список языков рекомендуется уточнять у специалистов технической поддержки Some Platform (support@example.com английский,`id`— индонезийский,`ms`— малайский,`my`— бирманский,`th`— тайский,`vi`— вьетнамский,`zh`— китайский.Код языка платежной страницы. |
...
ru_pm_thailand_banks_pp_purchase_redirect.ditaПодробная информация о проведении оплаты с использованием метода title через Payment Widget представлена в разделе.
...
| **Регионы использования** |  |
| --- | --- |
| **Валюты оплаты** | references/ru/currencies/THB.dita |
| **Суммы платежей** | Информацию уточняйте у курирующего менеджера Some Platform. Также вы можете уточнить в Dashboard минимальную и максимальную сумму платежа, доступную в вашем проекте. |
| **Время проведения платежа** | Информацию уточняйте у курирующего менеджера Some Platform. |
| **Конвертация валют** | На стороне Some Platform |
| **Возврат** |  |
| **Организация и стоимость подключения** | По согласованию с курирующим менеджером Some Platform |
| **Особенности**ru_pp_cascading.ditaДля работы с этим методом через Payment Widget поддерживается каскадное проведение оплат. Информацию о поддержке этой возможности можно получить в разделеи у специалистов технической поддержки Some Platform (support@example.com |  |
...
| --- | --- |
| **Формат тела запроса** | JSON |
| **Конечная точка** | `/v2/info/banks/thailand/sale/list` |
| **Полная спецификация конечной точки** |  |
