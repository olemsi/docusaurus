...
| Конечная точка | `/v2/payment/bank-transfer/colombia/sale` |
...
| phonetype_string type_optional | `1234567`Пример:Номер телефона пользователя. |  |
| identifytype_object type_strictly_required | doc_numbertype_string type_strictly_required | `1234567`Пример:Номер документа, подтверждающего личность пользователя. |
| doc_typetype_string type_strictly_required | Возможные значения:`CC`Пример:`CC`— Cédula de Ciudadanía — Свидетельство о гражданстве;`NIT`— Número de Identificación Tributaria — уникальный номер налогоплательщика Колумбии.Тип документа, подтверждающего личность пользователя. |  |
...
| --- | --- |
| Формат тела запроса | JSON |
| Конечная точка | `/v2/payment/bank-transfer/colombia/payout` |
| Полная спецификация конечной точки |  |
...
| last_nametype_string type_strictly_required | `Doe`Пример:Фамилия пользователя. |  |
| emailtype_string type_strictly_required | `johndoe@example.com`Пример:Адрес электронной почты пользователя. |  |
| phonetype_strictly_required✱type_string | `1234567890`Пример:Номер телефона пользователя, к которому привязан банковский счет, 10 цифр без знака плюс (+), пробелов и знаков пунктуации. |  |
| identifytype_object type_strictly_required | doc_numbertype_string type_strictly_required | `1234567`Пример:Номер документа, подтверждающего личность пользователя. |
| doc_typetype_strictly_required✱type_string | Возможные значения:`CC`Пример:`CC`— Cédula de Ciudadanía — Свидетельство о гражданстве;`NIT`— Número de Identificación Tributaria — уникальный номер налогоплательщика Колумбии.Тип документа, подтверждающего личность пользователя. |  |
| accounttype_object type_strictly_required | numbertype_string type_strictly_required | `1234567890`Пример:Номер счета пользователя, на который поступает выплата, или номер телефона, который привязан к счету пользователя. Уточняйте у вашего курирующего менеджера в Some Platform, что именно вам следует передавать в этом параметре. |
| typetype_strictly_required✱type_string | Возможные значения:`CACC`Пример:`CACC`— номер текущего счета;`SVGS`— номер сберегательного счета.Тип счета пользователя. |  |
| bank_idtype_strictly_required✱type_integer | ru_pm_colombia_bt_payout_banks.dita`12345`Пример:Идентификатор банка пользователя. Подробнее о том, как получить список поддерживаемых банков см.. |  |
| paymenttype_object type_strictly_required | currencytype_string type_strictly_required | `example_currency`Пример:Код валюты платежа в формате ISO-4217 alpha-3. |
| amounttype_integer type_strictly_required | *за исключением*Сумма платежа в дробных единицах валюты без десятичного разделителя (точки или запятой) и пробеловслучаев, когда у валюты нет дробной части.*основных единицах валюты*ru_currency_units.ditaru_currency_codes.ditaЕсли у валюты нет дробных единиц (то есть количество разрядов дробных единиц равно нулю), то в этом параметре нужно указывать сумму в. Подробнее о разрядах дробных единиц у валют см.и.`example_amount`Пример: example_amount_in_majors example_currency передается как |  |
| API ReferenceПри необходимости добавьте в запрос необязательные параметры, указанные в спецификации                     Gate:. |  |  |
...
| --- | --- |
| Banco Agrario | 70992 |
| Banco AV Villas | 71002 |
| Banco Caja Social BCSC SA | 71012 |
| Banco Colpatriam | 71022 |
| Banco Compartir S.A. | 71032 |
| Banco Cooperativo Coopcentral | 71042 |
| Banco Corpbanca Colombia S.A | 71052 |
| Banco Davivienda SA | 71062 |
| Banco de Bogotá | 71072 |
| Banco de Comercio Exterior | 71082 |
| Banco de las Microfinanzas Bancamia S.A. | 71092 |
| Banco de Occidente | 71102 |
| Banco Falabella S.A | 71112 |
| Banco Finandina S.A | 71122 |
| Banco GNB Sudameris | 71132 |
| Banco Multibank S.A | 71152 |
| Banco Mundo Mujer S.A. | 71162 |
| Banco Pichincha | 71182 |
| Banco Popular | 71192 |
| Banco Procredit Colombia | 71202 |
| Banco Santander de Negocios Colombia S.A. | 71212 |
| Banco Serfinanza S.A. | 71222 |
| Banco W S.A. | 71232 |
| Bancolombia | 71142 |
| Bancoomeva | 71172 |
| BBVA Colombia | 71242 |
| Citibank | 71252 |
| Coltefinanciera S.A | 71262 |
| Confiar Cooperativa Financiera | 71272 |
| Cooperativa Financiera de Antioquia | 71282 |
| Cootrafa Cooperativa Financiera | 71292 |
| Daviplata | 71302 |
| Financiera Juriscoop Compañía de Financiamiento | 71312 |
| Giros y Finanzas CF | 71322 |
| Itaú Corpbanca Colombia S.A. | 71332 |

---

## Bank transfers in Colombia -Выплата
...
| --- | --- |
| **Валюты выплаты** | references/ru/currencies/COP.ditareferences/ru/currencies/USD.dita, |
...
| customer_phonetype_string type_optional | `1234567890123`Пример:Номер телефона пользователя без знака +, только цифры, без пробелов и знаков препинания. |
| identify_doc_numbertype_string type_optional | `12345678901`Пример:Номер документа, подтверждающего личность пользователя, только цифры, без пробелов и знаков препинания. |
| identify_doc_typetype_string type_optional | Возможные значения:`CC`Пример:`CC`— Cédula de Ciudadanía — Свидетельство о гражданстве;`NIT`— Número de Identificación Tributaria — уникальный номер налогоплательщика Колумбии.Тип документа, подтверждающего личность пользователя. |
...
## Bank transfers in Colombia -Оплата
...
| **Регионы использования** |  |
| --- | --- |
| **Валюты оплаты** | references/ru/currencies/COP.ditareferences/ru/currencies/USD.dita, |
| **Суммы платежей** | Информацию уточняйте у курирующего менеджера Some Platform. Также вы можете уточнить в Dashboard минимальную и максимальную сумму платежа, доступную в вашем проекте. |
| **Время проведения платежа** | Информацию уточняйте у курирующего менеджера Some Platform. |
| **Конвертация валют** | На стороне Some Platform |
| **Возврат** |  |
| **Организация и стоимость подключения** | По согласованию с курирующим менеджером Some Platform |
| **Особенности**Если пользователь при оплате укажет сумму, отличную от суммы, указанной в запросе на оплату, то платеж будет отклонен. |  |
