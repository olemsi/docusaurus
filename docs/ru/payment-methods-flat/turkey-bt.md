...
| Конечная точка | `/v2/payment/bank-transfer/turkey/sale` |
...
| accounttype_strictly_required✱type_object | numbertype_strictly_required✱type_string | `1234567890`Пример:Номер счета пользователя. В качестве номера счета может передаваться IBAN пользователя. |
...
| Элемент | Описание |
| --- | --- |
| {     "type": "add_info",     "title": "receiver_name",     "data": "John Doe" } | `John Doe`Пример:Имя получателя платежа. |
| {     "type": "add_info",     "title": "receiver_account",     "data": "TR123456789012345" } | `TR123456789012345`Пример:Номер счета получателя платежа (IBAN). |
| {     "type": "add_info",     "title": "currency",     "data": "TRY" } | `TRY`Пример:Код валюты платежа в формате ISO-4217 alpha-3. |
...
| --- | --- |
| Формат тела запроса | JSON |
| Конечная точка | `/v2/payment/bank-transfer/turkey/payout` |
| Полная спецификация конечной точки |  |
...
| ip_addresstype_string type_strictly_required | `198.51.100.47`Пример:IP-адрес устройства пользователя. |  |
| emailtype_string type_strictly_required | `johndoe@example.com`Пример:Адрес электронной почты пользователя. |  |
| accounttype_object type_strictly_required | numbertype_string type_strictly_required | `1234567890`Пример:Номер счета пользователя. В качестве номера счета может передаваться IBAN пользователя. |
| bank_idtype_strictly_required✱type_integer | ru_pm_turkey_bt_payout_banks.dita`12345`Пример:Идентификатор банка пользователя. Подробнее о том, как получить список поддерживаемых банков см.. |  |
...
| QNB Finance | 76316 |
| TEB bank | 72242 |
| Türkiye Finans | 72232 |
| Vakıfbank | 426 |
| YapıKredi | 422 |
| Ziraat Bankasi | 69422 |
...
| --- | --- |
| **Формат тела запроса** | JSON |
| **Конечная точка** | `/v2/info/banks/bank-transfer/turkey/payout/list` |
| **Полная спецификация конечной точки** |  |
...
## Bank transfers in Turkey -Выплата
...
| --- | --- |
| **Валюты выплаты** | references/ru/currencies/TRY.ditareferences/ru/currencies/USD.dita, |
...
| customer_account_numbertype_strictly_required✱type_string | `1234567890`Пример:Номер счета пользователя (в качестве номера счета может передаваться IBAN пользователя). |
...
## Bank transfers in Turkey -Оплата 2: с отображением реквизитов
...
| **Регионы использования** |  |
| --- | --- |
| **Валюты оплаты** | references/ru/currencies/TRY.ditareferences/ru/currencies/USD.dita, |
| **Суммы платежей** | Информацию уточняйте у курирующего менеджера Some Platform. Также вы можете уточнить в Dashboard минимальную и максимальную сумму платежа, доступную в вашем проекте. |
| **Время проведения платежа** | Информацию уточняйте у курирующего менеджера Some Platform. |
| **Конвертация валют** | На стороне Some Platform |
| **Возврат** |  |
...
| --- | --- |
| Регионы использования |  |
| Валюты платежей | references/ru/currencies/TRY.ditareferences/ru/currencies/USD.dita, |
| Суммы платежей | Информацию уточняйте у курирующего менеджера Some Platform. Также вы можете уточнить в Dashboard минимальную и максимальную сумму платежа, доступную в вашем проекте. |
| Время проведения платежа | Информацию уточняйте у курирующего менеджера Some Platform. |
| Конвертация валют | На стороне Some Platform |
| Оплата |  |
