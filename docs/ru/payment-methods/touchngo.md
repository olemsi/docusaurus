...
| Конечная точка | `/v2/payment/wallet/touchngo/sale` |
...
ru_pm_touchngo_gate_purchase_redirect.ditaПодробная информация о проведении оплаты с использованием метода title через Gate представлена в разделе.
...
| --- | --- |
| Формат тела запроса | JSON |
| Конечная точка | `/v2/payment/wallet/touchngo/payout` |
| Полная спецификация конечной точки |  |
...
ru_pm_touchngo_gate_payout.ditaПодробная информация о проведении выплаты с использованием метода title через Gate представлена в разделе.
...
ru_pm_touchngo_pp_purchase_redirect.ditaПодробная информация о проведении оплаты с использованием метода title через Payment Widget представлена в разделе.
...
Если в обработке уже находится такое количество платежей, что в указанном диапазоне не удается найти свободного значения, то этот и все последующие запросы отклоняются платежной платформой, пока не завершится обработка хотя бы одного из текущих платежей с той же суммой.

Если указанное увеличение суммы платежа произойдет, то в оповещении веб-сервису будет передана измененная сумма.

Вот пример поведения платежной платформы при последовательном поступлении двух запросов с одинаковой суммой:

1. В платежную платформу поступает первый запрос на оплату 15,00 MYR. Так как это первый такой платеж, то платформа начинает выполнение платежа с суммой 15,00 MYR.
2. В платежную платформу поступает второй запрос на оплату 15,00 MYR и к этому моменту проведение первого платежа не закончено.
3. Так как платеж на сумму 15,00 MYR уже выполняется, платежная платформа изменяет сумму второго платежа на 15,01 MYR и начинает выполнять его именно с этой суммой.
...
| **Возврат** |  |
| **Организация и стоимость подключения** | По согласованию с курирующим менеджером Some Platform |
| **Особенности**Если пользователь при оплате укажет сумму, отличную от суммы, указанной в запросе на оплату, то платеж останется в промежуточном статусе, пока конфликт не будет разрешен вручную техническими специалистами Some Platform.Платеж отклоняется, если пользователь не совершит оплату в течение 15 минут с момента инициирования платежа.#ru_pm_touchngo_purchase_redirect/special_considerationsВ некоторых случаях платежная платформа может увеличить сумму платежа на величину от 0,01 до 0,99 MYR. Подробнее о причинах и механизме изменения суммы в разделе. |  |
