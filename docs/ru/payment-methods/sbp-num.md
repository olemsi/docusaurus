...
| Элемент | Описание |
| --- | --- |
| {     "type": "add_info",     "title": "amount",     "data": "100000" } | display_dataВсегда присутствует в массиве.`100000`Пример:Сумма платежа. |
| {     "type": "add_info",     "title": "currency",     "data": "RUB" } | display_dataВсегда присутствует в массиве.`RUB`Пример:Валюта платежа. |
| {     "type": "add_info",     "title": "receiver_name",     "data": "Mark Lane" } | display_dataВсегда присутствует в массиве.`Mark Lane`Пример:Имя получателя перевода. |
| {     "type": "add_info",     "title": "receiver_account",     "data": "12345678901234" } | display_dataВсегда присутствует в массиве.`12345678901234`Пример:Номер телефона получателя перевода. |
| {     "type": "add_info",     "title": "bank_name",     "data": "Sberbank" } | display_dataВсегда присутствует в массиве.`Sberbank`Пример:Название банка получателя перевода. |
| {     "type": "add_info",     "title": "lifetime",     "data": "720" } | display_dataМожет отсутствовать в массиве.`720`Пример:Время, в течение которого действительны платежные реквизиты. Указывается в секундах. |
| display_datareceiver_countryДополнительно в массивеможет передаваться параметр. |  |
| {     "type": "add_info",     "title": "receiver_country",     "data": "TJ" } | `TJ`Пример:Страна получателя перевода. |
| display_dataВ некоторых случаях массивможет содержать элементы, которые не указаны в этой таблице. Игнорируйте их. |  |
...
| paymenttype_object type_strictly_required | currencytype_string type_strictly_required | references/ru/currencies/RUB.ditaВ качестве валюты оплаты можно использовать только.`example_currency`Пример:Код валюты платежа в формате ISO-4217 alpha-3. |
| amounttype_integer type_strictly_required | *за исключением*Сумма платежа в дробных единицах валюты без десятичного разделителя (точки или запятой) и пробеловслучаев, когда у валюты нет дробной части.*основных единицах валюты*ru_currency_units.ditaru_currency_codes.ditaЕсли у валюты нет дробных единиц (то есть количество разрядов дробных единиц равно нулю), то в этом параметре нужно указывать сумму в. Подробнее о разрядах дробных единиц у валют см.и.`example_amount`Пример: example_amount_in_majors example_currency передается как |  |
| return_urltype_object type_strictly_required | successtype_string type_optional | `http://example.com/success/`Пример:URL-адрес, на который нужно перенаправить пользователя в случае успешного завершения оплаты. |
| declinetype_string type_optional | `http://example.com/decline/`Пример:URL-адрес, на который нужно перенаправить пользователя в случае отклонения оплаты. |  |
| returntype_string type_strictly_required | successdeclinereturn_url`http://example.com/return/`Пример:URL-адрес, куда нужно перенаправить пользователя в случае прерывания оплаты пользователем до ее завершения. Этот же адрес используется, если не заданы параметрыи. Если не задан ни один из параметров объекта, то Gate по умолчанию перенаправляет пользователя по URL-адресу, указанному в вашем проекте в Some Platform. |  |
| API ReferenceПри необходимости добавьте в запрос необязательные параметры, указанные в спецификации                      Gate:. |  |  |
...
| customertype_object type_strictly_required | idtype_string type_strictly_required | `customer_123`Пример:Идентификатор пользователя, уникальный в рамках проекта. |
| ip_addresstype_string type_strictly_required | `198.51.100.47`Пример:IP-адрес устройства пользователя. |  |
| first_nametype_string type_strictly_required | `Иван`Пример:Имя пользователя кириллицей, как указано в Системе быстрых переводов. |  |
| last_nametype_string type_strictly_required | `Иванов`Пример:Фамилия пользователя кириллицей, как указано в Системе быстрых переводов. |  |
| emailtype_string type_strictly_required | `ivanivanov@example.com`Пример:Адрес электронной почты пользователя. |  |
| middle_nametype_string type_strictly_required | `Иванович`Пример:Отчество пользователя кириллицей, как указано в Системе быстрых переводов. |  |
| accounttype_object type_strictly_required | numbertype_string type_strictly_required | `79876543210`Пример:Номер телефона пользователя для проведения платежа через Систему быстрых платежей, указывается с кодом страны, без знаков пунктуации. |
| bank_codetype_string type_strictly_required | [https://qr.nspk.ru/proxyapp/c2bmembers.json](https://qr.nspk.ru/proxyapp/c2bmembers.json)`123456789123`Пример:Код банка в Системе быстрых платежей, 12 цифр. Актуальные коды банков доступны по ссылке:. |  |
| paymenttype_object type_strictly_required | currencytype_string type_strictly_required | references/ru/currencies/RUB.ditaВ качестве валюты оплаты можно использовать только.`example_currency`Пример:Код валюты платежа в формате ISO-4217 alpha-3. |
| amounttype_integer type_strictly_required | *за исключением*Сумма платежа в дробных единицах валюты без десятичного разделителя (точки или запятой) и пробеловслучаев, когда у валюты нет дробной части.*основных единицах валюты*ru_currency_units.ditaru_currency_codes.ditaЕсли у валюты нет дробных единиц (то есть количество разрядов дробных единиц равно нулю), то в этом параметре нужно указывать сумму в. Подробнее о разрядах дробных единиц у валют см.и.`example_amount`Пример: example_amount_in_majors example_currency передается как |  |
| API ReferenceПри необходимости добавьте в запрос необязательные параметры, указанные в спецификации                     Gate:. |  |  |
...
account.bank_codeВыплата в методе title осуществляется через банки, поддерживающие работу с этим методом. В запросе на выплату в параметревы должны передать код банка пользователя, который следует использовать для выполнения платежа.

---

## «Платежи по номеру телефона через Систему быстрых платежей» -Выплата
...
| **Особенности**references/ru/currencies/RUB.ditaВ качестве валюты оплаты можно использовать только. |  |
...
## «Платежи по номеру телефона через Систему быстрых платежей» -Оплата 2: с отображением реквизитов
...
## «Платежи по номеру телефона через Систему быстрых платежей» -Оплата 1: перенаправление в сервис провайдера
