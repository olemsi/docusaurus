...
| last_nametype_string type_strictly_required | `Doe`Пример:Фамилия пользователя. |  |
| emailtype_string type_strictly_required | `johndoe@example.com`Пример:Адрес электронной почты пользователя. |  |
| accounttype_object type_strictly_required | numbertype_string type_strictly_required | `79876543210`Пример:Номер телефона пользователя, указывается с кодом страны, без знаков пунктуации. |
| bank_codetype_string type_strictly_required | ru_pm_p2p_num_kyrgyzstan_payout_banks.dita`123456789123`Пример:Код банка пользователя. Актуальные коды банков доступны в разделе. |  |
| paymenttype_object type_strictly_required | currencytype_string type_strictly_required | references/ru/currencies/KGS.ditaВ качестве валюты оплаты можно использовать только.`example_currency`Пример:Код валюты платежа в формате ISO-4217 alpha-3. |
| amounttype_integer type_strictly_required | *за исключением*Сумма платежа в дробных единицах валюты без десятичного разделителя (точки или запятой) и пробеловслучаев, когда у валюты нет дробной части.*основных единицах валюты*ru_currency_units.ditaru_currency_codes.ditaЕсли у валюты нет дробных единиц (то есть количество разрядов дробных единиц равно нулю), то в этом параметре нужно указывать сумму в. Подробнее о разрядах дробных единиц у валют см.и.`example_amount`Пример: example_amount_in_majors example_currency передается как |  |
| API ReferenceПри необходимости добавьте в запрос необязательные параметры, указанные в спецификации                     Gate:. |  |  |
...
## «Платежи по номеру телефона в Киргизии» -Выплата
...
| --- | --- |
| **Валюты выплаты** | references/ru/currencies/KGS.dita |
...
| **Особенности**references/ru/currencies/KGS.ditaВ качестве валюты выплаты можно использовать только. |  |
...
## «Платежи по номеру телефона в Киргизии» -Оплата
