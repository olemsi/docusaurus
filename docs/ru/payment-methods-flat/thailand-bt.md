...
| Конечная точка | `/v2/payment/bank-transfer/thailand/sale` |
...
| customer_first_nametype_string type_required | `John`Пример:Имя пользователя. |
| customer_last_nametype_string type_required | `Doe`Пример:Фамилия пользователя. |
...
## Bank transfers in Thailand -Оплата
