$(document).ready(function(){
    ...
    let secondLogoLink = "index.html"; //ссылка второго двойного лого
    ...
    let firstLogoImage_desktop = "dist/plugins/topMenu/images/logo.svg"; //ссылка на картинку первого двойного лого для десктопного вида
    let secondLogoImage_desktop = "dist/plugins/topMenu/images/documentation.svg"; //ссылка на картинку второго двойного лого для десктопного вида
    let firstLogoImage_mobile = "dist/plugins/topMenu/images/logo.svg"; //ссылка на картинку первого двойного лого для мобильного вида
    let secondLogoImage_mobile = "dist/plugins/topMenu/images/doc.svg"; //ссылка на картинку второго двойного лого для мобильного вида
    ...
    let pageLanguage = "en"; // ru для русского, все что угодня для английского
...
/*		{
        ...
        },
        {
            ...
            sectionDescription: "A portal to the world of Platform clients. Apply to get started, or contact us if you have any questions.",
            sectionHref: "#",
            sectionChildren: [{
                name: "Apply now",
                href: ""
    ...
    if(pageLanguage == "ru"){
        langSwitcher =`
            <div class="lang-switch lang-switch-ru">
        ...
        langSwitcher =`
            <div class="lang-switch lang-switch-en">
                <div class="lang-switch_lang lang-switch__change_lang ">
                    <a href="#">
                        <div class="lang-switch__icon-country lang-switch__icon-country_en"></div>
                        <span>EN</span>
...
<div class="header__wrapper ${operationSystem} ${browser}">
    ...
    <!-- КОНВЕРТ -->
    <a class="user-nav__write-btn" data-trigger=""></a>
    <!-- СМЕНА ЯЗЫКА -->
    ${langSwitcher}
    ...
    //открытие и закрытие меню на десктопе
    ...
    //открытие и закрытие меню на планшете
    $(".navbar-tablet .top-menu__menu-current-section, .navbar-tablet .top-menu__menu-section-items-container").on("mouseover touch",function(){
        if($('body').hasClass('mobile-search_active')){
            return;
        }
        $(".top-menu__layout").addClass("show-element");
    });
    $(".navbar-tablet .top-menu__menu-current-section, .navbar-tablet .top-menu__menu-section-items-container").on("mouseout",function(){
        if($('body').hasClass('mobile-search_active')){
            return;
        }
        $(".top-menu__layout").removeClass("show-element");
    ...
    $('body:not(.wh_search_page) .mobile-search-cancel, header, .top-menu__layout').on('click', function (e) {
        ...
        if($(e.target).hasClass("mobile-search-cancel")){
            e.stopPropagation();
        }
    ...
    function SelectActiveElement(menuTypeClass){
        ...
        if (breadcrumbsSectionsLinks.length > 0) {
            ...
            //отключено для главной страницы
            // let currentPage = window.location.pathname;
            // let regexp = new RegExp(currentPage, 'g');
    ...
    $(".user-nav__write-btn").on("click",function(e){
        e.preventDefault();
    });
...
$("header").on("contextmenu",".menu-animated-button, .menu-simple-marketing-button",function(){
    $(this).blur();
});
$("header").on("mouseup",".menu-animated-button, .menu-simple-marketing-button",function(event){
    if(event.which == 2) {
        $(this).blur();
    }
});
