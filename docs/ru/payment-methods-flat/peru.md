...
| Конечная точка | `/v2/payment/wallet/peru-qr/sale` |
...
| identifytype_object type_required | doc_numbertype_string type_required | `12345678`Пример:Номер документа, подтверждающего личность пользователя. Имеет ограничения по длине значения: 8 цифр — для документа DNI, от 7 до 12 цифр — для PAS, от 8 до 12 цифр — для CE, 11 цифр — для RUC. |
| doc_typetype_string type_required | `DNIPASCERUC`Возможные значения:,,и.`DNI`Пример:Тип документа, подтверждающего личность пользователя. |  |
...
| identify_doc_numbertype_string type_required | `12345678`Пример:Номер документа, подтверждающего личность пользователя. Имеет ограничения по длине значения: 8 цифр — для документа DNI, от 7 до 12 цифр — для PAS, от 8 до 12 цифр — для CE, 11 цифр — для RUC. |
| identify_doc_typetype_string type_required | `DNIPASCERUC`Возможные значения:,,и.`DNI`Пример:Тип документа, подтверждающего личность пользователя. |
...
| **Особенности**Провайдер устанавливает ограничение на общую сумму оплат для одного пользователя за день — не более 3000,00 USD, за месяц — не более 5000,00 USD.Платеж отклоняется, если пользователь не совершит оплату в течение 48 часов с момента его перенаправления в сервис provider_service. |  |
...
| Тип платежного метода | Электронные кошелькиПлатежи по QR-коду |
| --- | --- |
| Регионы использования |  |
