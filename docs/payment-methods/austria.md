...
Use this page if you need the Austria-specific variant of Open Banking and want the shortest path to a working integration.

## What this method is

Open Banking in Austria lets the customer pay from an Austrian bank account in EUR through the bank or provider flow instead of entering card data.
...
Compared with a standard card flow, the main differences are:
...
- the Austria Open Banking method is enabled for your project
...
If any of this is unclear, confirm the project setup with your Platform account manager first.

## Bank selection

Austria Open Banking requires a bank selection step.
...
```text
POST /v2/info/banks/austria/sale/list
```
...
Payment Widget is the simpler option when you do not need a custom bank-selection UI.

Use it if you want:

- hosted checkout UI
- built-in method and bank selection
- less frontend work around redirects
...
Host-to-host API is the better option when you need to control the request payload and bank selection from your own backend.

Use it if you want:

- your own checkout UI
- your own bank-selection logic
- direct control over request payloads and callbacks
