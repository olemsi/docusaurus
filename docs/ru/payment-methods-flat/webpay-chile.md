...
| phonetype_string type_strictly_required | `56912345678`Пример:Номер телефона пользователя с кодом страны, без специальных символов и знаков пунктуации. |  |
| identifytype_object type_required | doc_numbertype_string type_required | `12345678912345678K`Пример:илиНомер документа, удостоверяющего личность пользователя (CI или RUT). Должен содержать от 8 до 9 знаков (только цифр или цифр и буквы) без специальных символов и знаков пунктуации. |
...
| customer_phonetype_string type_strictly_required | `56912345678`Пример:Номер телефона пользователя с кодом страны, без специальных символов и знаков пунктуации. |
| identify_doc_numbertype_string type_required | `12345678912345678K`Пример:илиНомер документа, удостоверяющего личность пользователя (CI или RUT). Должен содержать от 8 до 9 знаков (только цифр или цифр и буквы) без специальных символов и знаков пунктуации. |
...
| --- | --- |
| **Валюты оплаты** | references/ru/currencies/CLP.ditareferences/ru/currencies/USD.dita, |
...
| При работе с title оплаты могут проводиться через мобильное приложение и через сервис переводов с использованием платежных карт.Платежный метод title позволяет принимать от пользователей оплату, используя Payment Widget и Gate.здесьЗагрузить логотип этого платежного метода в векторном формате можно. |  |
| --- | --- |
