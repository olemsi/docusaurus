...
| Конечная точка | `v2/payment/bank-transfer/multibanco/sale` |
...
| Элемент | Описание |
| --- | --- |
| {     "type": "add_info",     "title": "entity",     "data": "12345" } | `12345`Пример:Идентификатор провайдера. |
| {     "type": "add_info",     "title": "reference",     "data": "123456789" } | `123456789`Пример:Номер платежа на стороне сервиса провайдера. |
| {     "type": "add_info",     "title": "amount",     "data": "10000" } | `10000`Пример:Сумма платежа в дробных единицах валюты. |
| {     "type": "add_info",     "title": "currency",     "data": "EUR" } | `EUR`Пример:Код валюты платежа в формате ISO-4217 alpha-3. |

| Текст инструкции на португальском языке | Текст инструкции на английском языке |
| --- | --- |
| **Pagamento e outros serviçosPagamentos de serviços/compras**Na sua conta online ou caixa eletrônico, escolha, e depois. | **Payment and other servicesPayments of services/shopping**In your online account or ATM machine chooseand then. |
...
## Multibanco -Оплата 1: с отображением реквизитов
...
## Multibanco -Оплата 2: перенаправление в сервис провайдера
...
| **Возврат** |  |
| **Организация и стоимость подключения** | По согласованию с курирующим менеджером Some Platform |
| **Особенности**Этот сценарий оплаты доступен только для IP адресов из Португалии. Если пользователь будет использовать IP адрес другой страны, то ему отобразится страница с другими доступными методами оплаты. |  |
