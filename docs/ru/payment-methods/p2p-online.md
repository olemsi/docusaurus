...
| accounttype_strictly_required✱type_object | bank_idtype_strictly_required✱type_integer | ru_pm_p2p_online_purchase_instruction_banks.dita`76686`Пример:Идентификатор банка пользователя. Подробнее о том, как получить список поддерживаемых банков см.. |
...
| {     "type": "add_info",     "title": "bank_name",     "data": "Dushanbe City Bank" } | `Dushanbe City Bank`Пример:Название банка получателя перевода. |
...
| {     "type": "add_info",     "title": "lifetime",     "data": 1200 } | `1200`Пример:Срок действия инструкции в секундах. |

| Текст инструкции на азербайджанском языке | Текст инструкции на казахском языке |
| --- | --- |
| Kart məlumatlarını kopyalayın.Bank tətbiqinə gedin və karta köçürmə edin.Depozitin kreditləşdirilməsini gözləyin.Depozitinizi kreditləşdirmək üçün bir ödənişlə karta köçürmə edin. Əks təqdirdə, ödənişinizi müəyyənləşdirə bilməyəcəyik. Təfərrüatlar hər ödənişlə dəyişir. | Карта деректемелерін көшіріңіз.Банк қосымшасына өтіп, картаға ақша аударыңыз.Депозиттің есептелуін күтіңіз.Депозитіңізді есепке алу үшін картаға бір төлеммен аударым жасаңыз. Әйтпесе, біз сіздің төлеміңізді анықтай алмаймыз. Деректемелер әр төлем сайын өзгереді. |

| Текст инструкции на русском языке | Текст инструкции на узбекском языке |
| --- | --- |
| Скопируйте реквизиты.Зайдите в банковское приложение и совершите перевод на карту.Ожидайте зачисление депозита.Для зачисления вашего депозита совершите перевод на карту одним платежом. В противном случае мы не сможем идентифицировать ваш платеж. Реквизиты меняются с каждым платежом. | Karta tafsilotlarini nusxalash.Bank ilovasiga o'ting va kartaga pul o'tkazing.Depozitni kreditlashni kuting.Depozitingizni kreditlash uchun kartaga bitta to'lov bilan pul o'tkazmasini amalga oshiring. Aks holda, biz sizning to'lovingizni aniqlay olmaymiz. Tafsilotlar har bir to'lov bilan o'zgaradi. |
...
Вы можете предварительно выбрать на странице оплаты банк для пользователя. Для этого добавьте в запрос на открытие страницы оплаты параметры:

Вот пример данных из запроса на открытие страницы оплаты с использованием метода title и предварительным выбором банка для пользователя:
...
| payment_methods_optionstype_object type_optional | banks_idtype_integer type_optional | Возможные значения:`76686`Пример:`76686`— для Душанбе Сити Банка;`69492`— для Сбербанка;`69502`— для Т-Банка;`69532`references/ru/currencies/AZN.dita— для Kapital Bank (платежи в);`69552`references/ru/currencies/UZS.dita— для Kapitalbank (платежи в);`69542`— для Kaspi Bank.Идентификатор банка пользователя. |
...
| Банк | Идентификатор |
| --- | --- |
| Душанбе Сити Банк | 76686 |
| Сбербанк | 69492 |
| Т-Банк | 69502 |
| references/ru/currencies/AZN.ditaKapital Bank (платежи в) | 69532 |
| references/ru/currencies/UZS.ditaKapitalbank (платежи в) | 69552 |
| Kaspi Bank | 69542 |
...
| --- | --- |
| **Формат тела запроса** | JSON |
| **Конечная точка** | `/v2/info/banks/card-p2p/sale/list` |
...
## «Платежи посредством переводов с карты на карту через партнера» -Оплата 2: с отображением реквизитов
...
| **Регионы использования** | country_AZ, country_KZ, country_RU, country_TJ, country_UZ |
| --- | --- |
| **Валюты оплаты** | references/ru/currencies/AZN.ditareferences/ru/currencies/KZT.ditareferences/ru/currencies/RUB.ditareferences/ru/currencies/TJS.ditareferences/ru/currencies/UZS.dita,,,, |
...
| **Регионы использования** | country_AZ, country_IN, country_KZ, country_RU, country_TJ, country_UA, country_UZ |
| --- | --- |
| **Валюты оплаты** | references/ru/currencies/AZN.ditareferences/ru/currencies/INR.ditareferences/ru/currencies/KZT.ditareferences/ru/currencies/RUB.ditareferences/ru/currencies/TJS.ditareferences/ru/currencies/UAH.ditareferences/ru/currencies/UZS.dita,,,,,, |
| **Суммы платежей** | Информацию уточняйте у курирующего менеджера Some Platform. Также вы можете уточнить в Dashboard минимальную и максимальную сумму платежа, доступную в вашем проекте. |
| **Время проведения платежа** | Информацию уточняйте у курирующего менеджера Some Platform. |
| **Конвертация валют** | На стороне Some Platform |
| **Возврат** |  |
...
| title — это платежный метод, который позволит вам принимать оплату от пользователей. Пользователи выполняют оплату с использованием банковских переводов.Платежный метод title позволяет принимать от пользователей оплату, используя Payment Widget и Gate.здесьЗагрузить логотип этого платежного метода в векторном формате можно.здесьЗагрузить архивный файл с логотипами для стран, доступных в платежном методе, можно. |  |
| --- | --- |

| Тип платежного метода | Банковские переводы |
| --- | --- |
| Регионы использования | country_AZ, country_IN, country_KZ, country_RU, country_TJ, country_UA, country_UZ |
| Валюты платежей | references/ru/currencies/AZN.ditareferences/ru/currencies/INR.ditareferences/ru/currencies/KZT.ditareferences/ru/currencies/RUB.ditareferences/ru/currencies/TJS.ditareferences/ru/currencies/UAH.ditareferences/ru/currencies/UZS.dita,,,,,, |
| Суммы платежей | Информацию уточняйте у курирующего менеджера Some Platform. Также вы можете уточнить в Dashboard минимальную и максимальную сумму платежа, доступную в вашем проекте. |
| Время проведения платежа | Информацию уточняйте у курирующего менеджера Some Platform. |
| Конвертация валют | На стороне Some Platform |
| Оплата |  |
