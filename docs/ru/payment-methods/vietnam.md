...
В проведении отдельного платежа с использованием метода title задействуются веб-сервис мерчанта, один из интерфейсов и платежная платформа Some Platform, а также сервис одного из банков, поддерживающих работу с этим методом.
...
| Оплата |  |  |  | * | * | до 5 минут | 48 часов |
| Выплата |  |  |  | * | * | до 10 минут | 48 часов |
...
В методе title можно использовать следующие точки:
...
| Bank for Foreign Trade of Vietnam (VietcomBank) | 66361 |  |  |
...
| Hongleong Bank | 6291 |  |  |
| HSBC (Việt Nam) | 70372 |  |  |
| IBKHCM Công nghiệp Hàn Quốc - HCM | 70342 |  |  |
| IBKHN Công nghiệp Hàn Quốc - Hà Nội | 70392 |  |  |
...
| банки Вьетнамаtitle — платежный метод для проведения платежей через. Оплата осуществляется через Payment Widget и Gate, а выплата — через Gate и Dashboard.здесьЗагрузить логотип этого платежного метода в векторном формате можно. |  |
| --- | --- |
...
| Частичный возврат |  |
| Опротестование |  |
| Особенности | ru_pp_cascading.ditaДля работы с этим методом через Payment Widget поддерживается каскадное проведение оплат. Информацию о поддержке этой возможности можно получить в разделеи у курирующего менеджера Some PlatformДля валюты VND суммы указываются без дробной части |
...
| Конечная точка | `/v2/payment/banks/vietnam/sale` |
...
| accounttype_strictly_required✱type_object | numbertype_strictly_required✱type_string | `1234567890`Пример:Номер счета пользователя. |
| bank_idtype_strictly_required✱type_integer | ru_pm_vietnam_purchase_redirect_banks.dita`61021`Пример:Идентификатор банка пользователя. Подробнее о том, как получить список поддерживаемых банков см.. |  |
| customer_nametype_strictly_required✱type_string | `John Doe`Пример:Полное имя пользователя. |  |
...
ru_pm_vietnam_gate_purchase_redirect.ditaПодробная информация о проведении оплаты с использованием метода title через Gate представлена в разделе.
...
| --- | --- |
| Формат тела запроса | JSON |
| Конечная точка | `/v2/payment/banks/vietnam/payout` |
| Полная спецификация конечной точки |  |
...
| bank_idtype_integer type_strictly_required | ru_pm_vietnam_payout_banks.dita`61021`Пример:Идентификатор банка пользователя. Подробнее о том, как получить список поддерживаемых банков см.. |  |
| customer_nametype_string type_strictly_required | `Doe John`Пример:Полное имя пользователя. В связи с особенностями региона, необходимо передавать имя в следующем порядке: "Фамилия Имя". |  |
| paymenttype_object type_strictly_required | currencytype_string type_strictly_required | `example_currency`Пример:Код валюты платежа в формате ISO-4217 alpha-3. |
| amounttype_integer type_strictly_required | Сумма платежа в основных единицах валюты, без дробной части.*в дробных единицах валюты без десятичного разделителя (точки или запятой) и пробелов*ru_currency_units.ditaru_currency_codes.ditaЕсли у валюты есть дробные единицы (то есть количество разрядов дробных единиц не равно нулю), то в этом параметре нужно указывать сумму в. Подробнее о разрядах дробных единиц у валют см.и.`example_amount`Пример: example_amount_in_majors example_currency передается как |  |
| API ReferenceПри необходимости добавьте в запрос необязательные параметры, указанные в спецификации                     Gate:. |  |  |
...
ru_pm_vietnam_gate_payout.ditaПодробная информация о проведении выплаты с использованием метода title через Gate представлена в разделе.
...
| An Binh Commercial Joint Stock Bank (ABBANK) | 465 |
| Asia Commercial Bank | 150 |
| KASIKORNBANK Public Company Limited Ho Chi Minh City Branch | 70302 |
| Bac A Commercial Joint Stock Bank (Bac A Bank) | 439 |
| Bank for Foreign Trade of Vietnam (VietcomBank) | 66361 |
| Bank for Investment and Development of Vietnam | 136 |
| Bao Viet Bank | 453 |
| CBBank | 70382 |
| CIMB Bank (Vietnam) Limited | 6271 |
| CIMB | 66201 |
| Citibank N.A. | 454 |
| Co-op Bank of Vietnam | 70462 |
| Co-Operative Bank of Vietnam | 70292 |
| Dai Chung Viet Nam | 457 |
| DBS Bank Ltd - HCM | 70402 |
| Dong Nam A | 468 |
| DongA Joint Stock Commercial Bank | 130 |
...
| Global Petro Commercial Joint Stock Bank (GPBank) | 444 |
| HoChiMinh City Development Joint Stock Commercial Bank (HDBank) | 448 |
| Hong Leong Bank Vietnam Limited | 6291 |
| HSBC Bank (Viet Nam) | 70372 |
| HSBC Vietnam | 462 |
| Industrial Bank of Korea, Vietnam HCM | 70342 |
| Industrial Bank of Korea, Vietnam Hanoi | 70392 |
| Indovina Bank Ltd. (IVB) | 569 |
| KB Kookmin Bank | 458 |
| Kien Long | 455 |
| Kookmin - Hanoi | 70362 |
...
| Lien Doanh Viet Nga | 4691 |
| LienVietPostBank | 706 |
| Lien Viet Post Joint Stock Commercial Bank | 70472 |
| MBB | 66241 |
| Military JSC Bank (MB) | 447 |
| MTV Public Viet Nam | 3981 |
| Nam A commercial joint stock bank (NamABank) | 452 |
| National Citizen Commercial Joint Stock Bank (NCB) | 442 |
| NH số CAKE by VPBank | 70432 |
| NH số Ubank by VPBank | 70442 |
| NongHyup Bank Hanoi Branch | 70332 |
| North Asia Commercial Joint Stock Bank | 55841 |
| Ocean Commercial One Member Limited Liability Bank (OceanBank) | 450 |
| Orient Commercial Bank (Phuong Dong Bank) | 467 |
| Petrolimex Group Commercial Joint Stock Bank (PGBank) | 443 |
...
| Vietnam Maritime Commercial Joint Stock Bank (MaritimeBank) | 440 |
| Vietnam Prosperity Bank (VPBank) | 449 |
| Vietnam - Russia Joint Venture Bank (VRB) | 70312 |
| Woori Bank Vietnam | 1661 |
...
| --- | --- |
| **Формат тела запроса** | JSON |
| **Конечная точка** | `/v2/info/banks/vietnam/payout/list` |
| **Полная спецификация конечной точки** |  |
...
| --- | --- |
| **Валюты выплаты** | references/ru/currencies/VND.dita |
...
| **Возврат** |  |
| **Организация и стоимость подключения** | По согласованию с курирующим менеджером Some Platform |
| **Особенности**references/ru/currencies/VND.ditaСуммы в запросах с использованием валютыуказываются в основных единицах валюты в соответствии со стандартом ISO-4217 alpha-3.references/ru/currencies/VND.dita`1000`Например, при оплате на 1000в запросе нужно передавать значение.ru_currency_codes.ditaЧисло дробных единиц для различных валют указано в разделе. |  |
...
banks_idru_pm_vietnam_purchase_redirect_banks.ditaИдентификаторы банков, которые вы можете передавать в параметре, чтобы отобразить пользователю на странице оплаты значки конкретных банков, представлены в разделе.
...
ru_pm_vietnam_pp_purchase_redirect.ditaПодробная информация о проведении оплаты с использованием метода title через Payment Widget представлена в разделе.
...
| --- | --- |
| **Формат тела запроса** | JSON |
| **Конечная точка** | `/v2/info/banks/vietnam/sale/list` |
| **Полная спецификация конечной точки** |  |
