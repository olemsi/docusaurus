...
| Конечная точка | `/v2/payment/wallet/bkash/sale` |
...
`account.number`В этом методе в оповещении дополнительно передаётся параметр, содержащий номер счета пользователя.
...
| --- | --- |
| Формат тела запроса | JSON |
| Конечная точка | `/v2/payment/wallet/bkash/payout` |
| Полная спецификация конечной точки |  |
...
## bKash -Оплата 1: с отображением реквизитов
...
## bKash -Оплата 2: перенаправление в сервис провайдера
