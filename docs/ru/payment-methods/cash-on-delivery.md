...
| phonetype_string type_optional | `911234567890`Пример:Номер телефона пользователя, без знака +, без пробелов и знаков препинания. |  |
| addresstype_string type_optional | `Jalaram Jyot, Khandelwal Society, Shankar Lane, Kandivali West.`Пример:Адрес пользователя. |  |
| citytype_string type_optional | `Mumbai`Пример:Город пользователя. Список доступных городов уточняйте у специалистов технической поддержки Some Platform (support@example.com |  |
| statetype_string type_optional | `Maharashtra`Пример:Штат пользователя. Список доступных штатов уточняйте у специалистов технической поддержки Some Platform (support@example.com |  |
| ziptype_string type_optional | `400067`Пример:Почтовый индекс пользователя. |  |
...
| by_methodtype_string type_strictly_required | `cash_on_delivery`Пример:Код платежного метода. |  |
| return_urltype_object type_strictly_required | successtype_string type_strictly_required | successdeclinereturnВ запросе должны использоваться либо параметрыи, либо только параметр.`https://example.com/success/`Пример:URL-адрес, на который нужно перенаправить пользователя в случае успешного завершения оплаты. |
| declinetype_string type_strictly_required | successdeclinereturnВ запросе должны использоваться либо параметрыи, либо только параметр.`https://example.com/decline/`Пример:URL-адрес, на который нужно перенаправить пользователя в случае отклонения оплаты. |  |
| returntype_string type_strictly_required | successdeclinereturn_urlsuccessdeclinereturnВ запросе должны использоваться либо параметрыи, либо только параметр.`https://example.com/return/`Пример:URL-адрес, куда нужно перенаправить пользователя в случае прерывания оплаты пользователем до ее завершения. Этот же адрес используется, если не заданы параметрыи. Если не задан ни один из параметров объекта, то Gate по умолчанию перенаправляет пользователя по URL-адресу, указанному в вашем проекте в Some Platform. |  |
...
| last_nametype_string type_strictly_required | `Doe`Пример:Фамилия пользователя. |  |
| emailtype_string type_strictly_required | `johndoe@example.com`Пример:Адрес электронной почты пользователя. |  |
| phonetype_string type_strictly_required | `911234567890`Пример:Номер телефона пользователя, без знака +, без пробелов и знаков препинания. |  |
| addresstype_string type_strictly_required | `Jalaram Jyot, Khandelwal Society, Shankar Lane, Kandivali West.`Пример:Адрес пользователя. |  |
| citytype_string type_strictly_required | `Mumbai`Пример:Город пользователя. Список доступных городов уточняйте у специалистов технической поддержки Some Platform (support@example.com |  |
| statetype_string type_strictly_required | `Maharashtra`Пример:Штат пользователя. Список доступных штатов уточняйте у специалистов технической поддержки Some Platform (support@example.com |  |
| ziptype_string type_strictly_required | `400067`Пример:Почтовый индекс пользователя. |  |
| paymenttype_object type_strictly_required | currencytype_string type_strictly_required | `example_currency`Пример:Код валюты платежа в формате ISO-4217 alpha-3. |
| amounttype_integer type_strictly_required | *за исключением*Сумма платежа в дробных единицах валюты без десятичного разделителя (точки или запятой) и пробеловслучаев, когда у валюты нет дробной части.*основных единицах валюты*ru_currency_units.ditaru_currency_codes.ditaЕсли у валюты нет дробных единиц (то есть количество разрядов дробных единиц равно нулю), то в этом параметре нужно указывать сумму в. Подробнее о разрядах дробных единиц у валют см.и.`example_amount`Пример: example_amount_in_majors example_currency передается как |  |
| by_methodtype_string type_strictly_required | `cash_on_delivery`Пример:Код платежного метода. |  |
| API ReferenceПри необходимости добавьте в запрос необязательные параметры, указанные в спецификации                     Gate:. |  |  |
...
## Cash on delivery -Выплата

При выплате пользователь инициирует платеж и затем встречается с курьером сервиса title для получения денежных средств.
...
| **Особенности**Сервис provider_service обрабатывает запросы по будням с 9:00 до 21:00, кроме дней национальных праздников. Запросы, поступившие в нерабочее время, собираются в очередь и обрабатываются на следующий рабочий день.Запросы, поступившие в период с 19:00 до 21:00 обрабатываются на следующий рабочий день.Возможность проведения выплаты через Dashboard уточняйте у службы поддержки Some Platform. |  |
...
| customer_phonetype_string type_optional | `911234567890`Пример:Номер телефона пользователя, без знака +, без пробелов и знаков препинания. |
...
| customer_addresstype_string type_optional | `Jalaram Jyot, Khandelwal Society, Shankar Lane, Kandivali West.`Пример:Адрес пользователя. |
| customer_citytype_string type_optional | `Mumbai`Пример:Город пользователя. Список доступных городов уточняйте у специалистов технической поддержки Some Platform (support@example.com |
| customer_statetype_string type_optional | `Maharashtra`Пример:Штат пользователя. Список доступных штатов уточняйте у специалистов технической поддержки Some Platform (support@example.com |
| customer_ziptype_string type_optional | `400067`Пример:Почтовый индекс пользователя. |
...
## Cash on delivery -Оплата

Проведение оплаты выполняется с перенаправлением пользователя в сервис provider_service и личной встречей пользователя с курьером сервиса title для передачи денежных средств.
