...
| Конечная точка | `/v2/payment/wallet/yape/sale` |
...
| phonetype_string type_required | `51912345678`Пример:Номер телефона пользователя, 11 цифр без знака +, без пробелов и знаков препинания. |  |
| identifytype_object type_required | doc_numbertype_string type_required | doc_type`DNIPASCERUC``12345678`Пример:Номер документа, подтверждающего личность пользователя. Номер документа должен соответствовать типу, передаваемому в параметре. Только цифры, без пробелов и знаков препинания. Имеет ограничения по длине значения: 8 цифр — для документа, от 7 до 12 цифр — для, от 8 до 12 цифр — для, 11 цифр — для. |
| doc_typetype_string type_required | Возможные значения:`DNI`Пример:`PAS`— Номер паспорта пользователя;`CE`— Национальный документ, удостоверяющий личность иностранца, проживающего в Перу (Carnet de Extranjería)`RUC`— Идентификатор налогоплательщика (Registro Único de Contribuyente).`DNI`— Национальный документ, удостоверяющий личность (Documento Nacional de Identidad)Тип документа, подтверждающего личность пользователя. |  |
...
| customer_phonetype_string type_required | `51912345678`Пример:Номер телефона пользователя, 11 цифр без знака +, без пробелов и знаков препинания. |
| identify_doc_numbertype_string type_required | identify_doc_type`DNIPASCERUC``12345678`Пример:Номер документа, подтверждающего личность пользователя. Номер документа должен соответствовать типу, передаваемому в параметре. Только цифры, без пробелов и знаков препинания. Имеет ограничения по длине значения: 8 цифр — для документа, от 7 до 12 цифр — для, от 8 до 12 цифр — для, 11 цифр — для. |
