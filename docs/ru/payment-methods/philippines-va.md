...
| Конечная точка | `/v2/payment/bank-transfer/philippines-va/sale` |
...
| Элемент | Описание |
| --- | --- |
| {     "type": "add_info",     "title": "account_number",     "data": "abcd_1234" } | `abcd_1234`Пример:Номер счета получателя перевода. |
| {     "type": "add_info",     "title": "expired_timestamp",     "data": "1684918220" } | `1684918220`Пример:Момент истечения срока действия платежа. Указывается в формате Unix-времени. |

| Example of the payment instruction text |
| --- |
| Open your Gcash or Maya appChoose «Transfer», then select «Netbank»Enter your name, given account number and amount, then confirmGcash or Maya to NetbankOpen your banking appChoose «Transfer to other banks», then select NetbankEnter your name, given account number and amount, then confirmOnline bankingGo to a remittance center near youRemit to NetbankProvide your name and given account number, then payCash through remittance servicesPlease pay using one of the following payment options: |
...
## Philippines Virtual Accounts -Оплата 1: с отображением реквизитов
...
| **Особенности**InstaPayPESONet**Время проведения оплаты**С&#160;минимальной задержкойЗапросы, поступившие в систему с 9:31 до 15:30, обрабатываются до 22:00. Запросы, поступившие в систему с 15:31 текущего дня до 9:30 следующего дня, обрабатываются до 13:00 следующего дня. Все времена указаны для зоны UTC+8.**Максимальная сумма оплаты**references/ru/currencies/PHP.dita50&#160;000,00Без ограниченийПодробности о проведении платежей через PESONet и Instapay уточняйте у своего курирующего менеджера Some Platform.В методе title оплаты проводятся через сервисы InstaPay и PESONet. Вот основные различия этих сервисов:Передаваемый в платежной инструкции номер счета для перевода можно использовать только один раз. Повторные переводы на данный счет не будут зачислены.Если пользователь при оплате укажет сумму, отличную от суммы, указанной в запросе на оплату, то платеж останется в промежуточном статусе, пока конфликт не будет разрешен вручную техническими специалистами Some Platform. |  |

---

## Philippines Virtual Accounts -Оплата 2: перенаправление в сервис провайдера
...
| **Возврат** |  |
| **Организация и стоимость подключения** | По согласованию с курирующим менеджером Some Platform |
| **Особенности**Чтобы завершить платеж, пользователь должен загрузить чек из своего банковского приложения в специальное поле на странице оплаты, отображаемой на стороне сервиса провайдера.ru_gate_amount_modification.ditaЕсли пользователь при оплате укажет сумму, отличную от суммы, указанной в запросе на оплату, то платеж будет проводиться с особенностями, которые описаны в разделе.Платеж отклоняется, если пользователь не совершит оплату в течение 25 минут с момента его перенаправления в сервис provider_service. |  |
