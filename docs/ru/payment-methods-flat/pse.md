...
| Конечная точка | `/v2/payment/bank-transfer/pse/sale` |
...
| phonetype_required✱type_string | `1234567`Пример:Номер телефона пользователя (только цифры). |  |
| addresstype_required✱type_string | `20 Baltimore St.`Пример:Адрес пользователя (улица, номер дома, номер корпуса, номер квартиры и т. п.). Должен содержать только буквы, цифры, знаки пробела. |  |
| citytype_required✱type_string | `London`Пример:Город пользователя. |  |
| identifytype_required✱type_object | doc_numbertype_required✱type_string | `1234567`Пример:Номер документа, подтверждающего личность пользователя. |
| doc_typetype_required✱type_string | Возможные значения:`CC`Пример:`CC`— Cédula de Ciudadanía — Свидетельство о гражданстве;`NIT`— Número de Identificación Tributaria — уникальный номер налогоплательщика Колумбии.Тип документа, подтверждающего личность пользователя. |  |
| streettype_string type_optional | `Baltimore St.`Пример:Улица проживания пользователя. |  |
| accounttype_object type_optional | bank_idtype_integer type_optional | ru_pm_pse_purchase_redirect_banks.dita`12345`Пример:Идентификатор банка пользователя. Подробнее о том, как получить список поддерживаемых банков см.. |
...
| customer_phonetype_required✱type_string | `573211234567`Пример:Номер телефона пользователя (только цифры). |
| identify_doc_numbertype_required✱type_string | `123456789`Пример:Номер документа, подтверждающего личность пользователя: СС (номер карты гражданства) или NIT (идентификационный номер налогоплательщика). |
...
| customer_citytype_required✱type_string | `London`Пример:Город проживания пользователя. |
| customer_addresstype_required✱type_string | `20 Baltimore St.`Пример:Домашний адрес пользователя. |
