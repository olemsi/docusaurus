...
| Конечная точка | `/v2/payment/wallet/ovo/sale` |
...
| accounttype_object type_strictly_required | numbertype_string type_strictly_required | `081211111111`Пример:Идентификатор учетной записи OVO |
...
ru_pm_ovo_gate_purchase_redirect.ditaПодробная информация о проведении оплаты с использованием метода title через Gate представлена в разделе.
...
| --- | --- |
| Формат тела запроса | JSON |
| Конечная точка | `/v2/payment/wallet/ovo/payout` |
| Полная спецификация конечной точки |  |
...
| customer_account_numbertype_string type_optional | `081211111111`Пример:Идентификатор учетной записи OVO |
...
ru_pm_ovo_pp_purchase_redirect.ditaПодробная информация о проведении оплаты с использованием метода title через Payment Widget представлена в разделе.
...
| **Возврат** |  |
| **Организация и стоимость подключения** | По согласованию с курирующим менеджером Some Platform |
| **Особенности**references/ru/currencies/IDR.ditareferences/ru/currencies/IDR.ditareferences/ru/currencies/IDR.ditareferences/ru/currencies/IDR.ditaru_currency_codes.ditaСумму в запросе указывайте в дробных единицах валюты! Например, сумма 95,00в запросе представляется как 9500. Число дробных единиц для различных валют можно узнать в разделе.Сумма платежа в валютедолжна быть целочисленной, иначе дробная часть будет отсечена при обработке запроса в платежной платформе. Например, если в запросе указана сумма 95,50, то будет выполнен платеж на сумму 95,00.references/ru/currencies/IDR.ditaПри указании в запросах других валют сумма платежа конвертируется на стороне Some Platform с округлением до 1 000,00. |  |
