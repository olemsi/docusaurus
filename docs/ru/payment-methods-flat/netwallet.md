...
| Конечная точка | `/v2/payment/wallet/netwallet/sale` |
...
| accounttype_object type_strictly_required | numbertype_string type_strictly_required | `login@example.com`Пример:Логин пользователя в сервисе NetWallet. |
| security_codetype_string type_strictly_required | `password123`Пример:Пароль пользователя в сервисе NetWallet. |  |
...
| --- | --- |
| Формат тела запроса | JSON |
| Конечная точка | `/v2/payment/wallet/netwallet/payout` |
| Полная спецификация конечной точки |  |
...
| **Особенности**Прежде чем инициировать выплату в первый раз, незарегистрированные пользователи должны сначала пройти регистрацию в сервисе NetWallet. |  |
...
| customer_security_codetype_string type_strictly_required | `password123`Пример:Пароль пользователя в сервисе NetWallet. |
| account_numbertype_string type_strictly_required | `login@example.com`Пример:Логин пользователя в сервисе NetWallet. |
...
| **Возврат** |  |
| **Организация и стоимость подключения** | По согласованию с курирующим менеджером Some Platform |
| **Особенности**Прежде чем совершать оплату в первый раз, незарегистрированные пользователи должны сначала пройти регистрацию в сервисе NetWallet.Платеж отклоняется, если пользователь не совершит оплату в течение 10 минут с момента инициирования платежа. |  |
