...
| Конечная точка | `/v2/payment/online-banking/interac-cpi/sale` |
...
| phonetype_string type_required | `11234567890`Пример:Номер телефона пользователя, без знака +, без пробелов и знаков препинания. |  |
| accounttype_strictly_required✱type_object | provincetype_strictly_required✱type_string | Возможные значения:`AB`Пример:`AB`— для провинции Альберта`ON`— для провинции Онтарио`BC`— для провинции Британская Колумбия`MB`— для провинции Манитоба`SK`— для провинции Саскачеван`QC`— для провинции Квебек`NS`— для провинции Новая Шотландия`NB`— для провинции Нью-Брансуик`NL`— для провинции Ньюфаундленд и Лабрадор`PE`— для провинции Остров Принца Эдуарда`NT`— для Северо-Западных территорий`NU`— для территории Нунавут`YT`— для территории ЮконПровинция или территория пользователя. |
| paymenttype_object type_strictly_required | currencytype_string type_strictly_required | references/ru/currencies/CAD.ditaВ качестве валюты оплаты можно использовать только.`example_currency`Пример:Код валюты платежа в формате ISO-4217 alpha-3. |
...
first_namelast_namephoneemailip_addressВ методе title в оповещении дополнительно передаются параметры с данными о пользователе:— с именем пользователя,— с фамилией пользователя,— с номером телефона пользователя,— с адресом электронной почты пользователя,— с ip-адресом пользователя.
...
| customer_phonetype_string type_required | `11234567890`Пример:Номер телефона пользователя, без знака +, без пробелов и знаков препинания. |
| payment_currencytype_string type_strictly_required | references/ru/currencies/CAD.ditaВ качестве валюты оплаты можно использовать только.`example_currency`Пример:Код валюты платежа в формате ISO-4217 alpha-3. |
...
| account_provincetype_strictly_required✱type_string | Возможные значения:`AB`Пример:`AB`— для провинции Альберта`ON`— для провинции Онтарио`BC`— для провинции Британская Колумбия`MB`— для провинции Манитоба`SK`— для провинции Саскачеван`QC`— для провинции Квебек`NS`— для провинции Новая Шотландия`NB`— для провинции Нью-Брансуик`NL`— для провинции Ньюфаундленд и Лабрадор`PE`— для провинции Остров Принца Эдуарда`NT`— для Северо-Западных территорий`NU`— для территории Нунавут`YT`— для территории ЮконПровинция или территория пользователя. |
...
## Interac Combined Pay-in -Оплата
...
| --- | --- |
| **Валюты оплаты** | references/ru/currencies/CAD.dita |
...
| **Особенности**references/ru/currencies/CAD.ditaВ качестве валюты оплаты можно использовать только.Необходимо использовать внутренний гид по стилю Interac Combined Pay-in при разработке отображаемой пользователю платежной страницы. |  |
...
| title — это платежный метод, который позволит вам принимать оплату от пользователей.Платежный метод title позволяет принимать от пользователей оплату, используя Payment Widget и Gate.здесьЗагрузить логотип этого платежного метода в векторном формате можно. |  |
| --- | --- |

| Тип платежного метода | Банковские переводыИнтернет-банкинг |
| --- | --- |
| Регионы использования |  |
