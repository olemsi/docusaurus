...
| Конечная точка | `/v2/payment/bank-transfer/peru/sale` |
...
| phonetype_strictly_required✱type_string | `12345678901`Пример:Номер телефона пользователя. |  |
| identifytype_strictly_required✱type_object | doc_numbertype_strictly_required✱type_string | `12345678`Пример:Номер документа, подтверждающего личность пользователя. Имеет ограничения по длине значения: 8 цифр — для документа DNI, от 7 до 12 цифр — для PAS, от 8 до 12 цифр — для CE, 11 цифр — для RUC. |
| doc_typetype_strictly_required✱type_string | `DNIPASCERUC`Возможные значения:,,и.`DNI`Пример:Тип документа, подтверждающего личность пользователя. |  |
...
| --- | --- |
| Формат тела запроса | JSON |
| Конечная точка | `/v2/payment/bank-transfer/peru/payout` |
| Полная спецификация конечной точки |  |
...
| last_nametype_string type_strictly_required | `Doe`Пример:Фамилия пользователя. |  |
| emailtype_string type_strictly_required | `johndoe@example.com`Пример:Адрес электронной почты пользователя. |  |
| phonetype_strictly_required✱type_string | `912345678`Пример:Номер телефона пользователя, 9 цифр без кода страны. |  |
| identifytype_strictly_required✱type_object | doc_numbertype_strictly_required✱type_string | `12345678`Пример:Номер документа, подтверждающего личность пользователя. Имеет ограничения по длине значения: 8 цифр — для документа DNI, от 7 до 12 цифр — для PAS, от 8 до 12 цифр — для CE, 11 цифр — для RUC. |
| doc_typetype_strictly_required✱type_string | `DNIPASCERUC`Возможные значения:,,и.`DNI`Пример:Тип документа, подтверждающего личность пользователя. Уточните у курирующего менеджера Some Platform, какие именно типы документов актуальны для вас. |  |
| accounttype_object type_strictly_required | numbertype_string type_strictly_required | `CCI``123456789012345678`Пример:Номер(Código de Cuenta Interbancario) счета пользователя. Код из 18 или 20 цифр, используемый банками Перу для идентификации и направления денежных переводов в соответствующий банк и на банковский счет. |
| typetype_string type_strictly_required | `CACCSVGS`Возможные значения:и.`CACC`Пример:Тип счета пользователя. |  |
| bank_idtype_strictly_required✱type_integer | ru_pm_peru_bt_payout_banks.dita`70202`Пример:Идентификатор банка пользователя. Подробнее о том, как получить список поддерживаемых банков см.. |  |
| paymenttype_object type_strictly_required | currencytype_string type_strictly_required | `example_currency`Пример:Код валюты платежа в формате ISO-4217 alpha-3. |
| amounttype_integer type_strictly_required | *за исключением*Сумма платежа в дробных единицах валюты без десятичного разделителя (точки или запятой) и пробеловслучаев, когда у валюты нет дробной части.*основных единицах валюты*ru_currency_units.ditaru_currency_codes.ditaЕсли у валюты нет дробных единиц (то есть количество разрядов дробных единиц равно нулю), то в этом параметре нужно указывать сумму в. Подробнее о разрядах дробных единиц у валют см.и.`example_amount`Пример: example_amount_in_majors example_currency передается как |  |
| API ReferenceПри необходимости добавьте в запрос необязательные параметры, указанные в спецификации                     Gate:. |  |  |
...
| --- | --- |
| Alfin | 83882 |
| BANCO BBVA PERU | 59951 |
| Banco de Comercio | 70052 |
| Banco de Credito | 70112 |
| Banco de la Nacion | 70092 |
| Banco Falabella | 70012 |
| Banco Financiero | 70062 |
| Banco GNB | 70072 |
| BANCO INTERAMERICANO DE FINANZAS | 59981 |
| Banco Pichincha | 70122 |
| Banco Ripley | 70132 |
| Banco Santander | 70032 |
| BANCO SCOTIABANK PERU | 59971 |
| Caja Arequipa | 70172 |
| Caja Cuzco | 76266 |
| Caja Huancayo | 70192 |
| Caja Metropolitana | 70212 |
| Caja Piura | 70232 |
| Caja Sullana | 70242 |
| Caja Trujillo | 70262 |
| Citibank | 70082 |
| CMAC ICA (Caja Municipal de Ahorro Y Credito Ica) | 84052 |
| Compartamos Fin | 76186 |
| CREDISCOTIA | 76116 |
| FINANCIERA CONFIANZA | 84042 |
| FINANCIERA EFECTIVA | 84032 |
| FINANCIERA OH | 84022 |
| Interbank | 70102 |
| Mi Banco | 70042 |
...
| --- | --- |
| **Формат тела запроса** | JSON |
| **Конечная точка** | `/v2/info/banks/bank-transfer/peru/payout/list` |
| **Полная спецификация конечной точки** |  |
...
## Bank transfers in Peru -Выплата
...
| --- | --- |
| **Валюты выплаты** | references/ru/currencies/PEN.dita |
...
| customer_phonetype_string type_optional | `12345678901`Пример:Номер телефона пользователя. |
| identify_doc_numbertype_string type_optional | `12345678`Пример:Номер документа, подтверждающего личность пользователя. Имеет ограничения по длине значения: 8 цифр — для документа DNI, от 7 до 12 цифр — для PAS, от 8 до 12 цифр — для CE, 11 цифр — для RUC. |
| identify_doc_typetype_string type_optional | `DNIPASCERUC`Возможные значения:,,и.`DNI`Пример:Тип документа, подтверждающего личность пользователя. |
...
| --- | --- |
| Регионы использования |  |
| Валюты платежей | references/ru/currencies/PEN.dita |
| Суммы платежей | Информацию уточняйте у курирующего менеджера Some Platform. Также вы можете уточнить в Dashboard минимальную и максимальную сумму платежа, доступную в вашем проекте. |
| Время проведения платежа | Информацию уточняйте у курирующего менеджера Some Platform. |
| Конвертация валют | На стороне Some Platform |
| Оплата |  |
