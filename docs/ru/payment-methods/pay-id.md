...
| Конечная точка | `/v2/payment/bank-transfer/australia-payid/sale` |
...
| Элемент | Описание |
| --- | --- |
| {     "type": "add_info",     "title": "receiver_account",     "data": "johndoe@example.com" } | `johndoe@example.com`Пример:PayID получателя платежа. |
| {     "type": "add_info",     "title": "operation_id",     "data": "1234567890" } | `1234567890`Пример:Идентификатор операции. |
| {     "type": "add_info",     "title": "expired_timestamp",     "data": "1234567890" } | `1234567890`Пример:Время жизни реквизитов для оплаты. |
| {     "type": "add_info",     "title": "amount",     "data": "10000" } | `10000`Пример:Сумма платежа. |
| {     "type": "add_info",     "title": "currency",     "data": "AUD" } | `AUD`Пример:Валюта платежа. |
...
| --- | --- |
| Формат тела запроса | JSON |
| Конечная точка | `/v2/payment/bank-transfer/australia-payid/payout` |
| Полная спецификация конечной точки |  |
...
| last_nametype_string type_strictly_required | `Doe`Пример:Фамилия пользователя. |  |
| emailtype_string type_strictly_required | `johndoe@example.com`Пример:Адрес электронной почты пользователя. |  |
| accounttype_object type_strictly_required | numbertype_string type_strictly_required | `610123456789johndoe@example.com`Пример:илиPayID пользователя — номер мобильного телефона или адрес электронной почты. |
...
account.numberВ этом методе в оповещении дополнительно передаётся параметрс номером мобильного телефона или адресом электронной почты пользователя.
...
## PayID -Оплата 2: с отображением реквизитов
...
| **Регионы использования** |  |
| --- | --- |
| **Валюты оплаты** | references/ru/currencies/AUD.dita |
| **Суммы платежей** | Информацию уточняйте у курирующего менеджера Some Platform. Также вы можете уточнить в Dashboard минимальную и максимальную сумму платежа, доступную в вашем проекте. |
| **Время проведения платежа** | Информацию уточняйте у курирующего менеджера Some Platform. |
| **Конвертация валют** | На стороне Some Platform |
| **Возврат** |  |
...
## PayID -Оплата 1: перенаправление в сервис провайдера
...
| **Возврат** |  |
| **Организация и стоимость подключения** | По согласованию с курирующим менеджером Some Platform |
| **Особенности**Если пользователь при оплате укажет неверную сумму, платеж будет отклонен, а переведенные пользователем средства будут возвращены в течение нескольких дней (в зависимости от банка пользователя). |  |
