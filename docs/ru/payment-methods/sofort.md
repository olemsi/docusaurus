...
| Конечная точка | `/v2/payment/unify/sale` |
...
| addresstype_string type_strictly_required | `4 Privet Drive`Пример:Адрес пользователя (улица, номер дома, номер корпуса, номер квартиры и т.п.). |  |
| citytype_string type_strictly_required | `Little Whinging`Пример:Город пользователя. |  |
...
| by_methodtype_string type_strictly_required | sofort`sofort`Пример:Код платежного метода (его значение всегда). |  |
...
| customer_citytype_string type_strictly_required | `Little Whinging`Пример:Город пользователя. |
| customer_ziptype_string type_strictly_required | `123456`Пример:Почтовый индекс пользователя. |
| customer_addresstype_string type_strictly_required | `4 Privet Drive`Пример:Адрес пользователя (улица, номер дома, номер корпуса, номер квартиры и т.п.). |
...
| **Регионы использования** | country_AT, country_BE, country_CH, country_DE, country_ES, country_FI, country_FR, country_GB, country_IT, country_NL, country_NO, country_SE |
| --- | --- |
