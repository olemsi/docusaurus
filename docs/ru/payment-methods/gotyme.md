...
| Конечная точка | `/v2/payment/wallet/gotyme/sale` |
...
| --- | --- |
| Формат тела запроса | JSON |
| Конечная точка | `/v2/payment/wallet/gotyme/payout` |
| Полная спецификация конечной точки |  |
...
| **Возврат** |  |
| **Организация и стоимость подключения** | По согласованию с курирующим менеджером Some Platform |
| **Особенности**Сумма платежа, отображаемая в сервисе provider_service, отличается в меньшую сторону от суммы, которая была передана в запросе на проведение платежа. Пользователь должен произвести оплату согласно информации в сервисе provider_service, при этом мерчанту зачислится сумма, указанная в сервисе provider_service.Например:references/ru/currencies/PHP.ditaЕсли пользователь совершит перевод с указанием иной суммы, например укажет первоначальную сумму платежа (100,00&#160;), проведение платежа приостанавливается. В такой ситуации обращайтесь в техническую поддержку Some Platform (support@example.com сумма, которая была указана в исходном запросе на проведение платежа;references/ru/currencies/PHP.ditareferences/ru/currencies/PHP.ditareferences/ru/currencies/PHP.dita99,87&#160;— сумма, которая отображается пользователю. Пользователь должен совершить оплату на сумму 99,87&#160;, при этом мерчанту также зачислится ровно эта сумма (99,87&#160;). |  |
