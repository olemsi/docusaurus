...
| Конечная точка | `/v2/payment/bank-transfer/laos/sale` |
...
| accounttype_object type_required | customer_nametype_string type_required | `John Doe`Пример:Полное имя пользователя. |
...
## Отображене платежной инструкции пользователю
...
| { 	"type": "add_info", 	"title": "payment_amount_show", 	"data": "500000.00" } | display_dataВсегда присутствует в массиве.`500000.00`Пример:Сумма платежа. |
| { 	"type": "add_info", 	"title": "bank_code", 	"data": "BCEL" } | display_dataВсегда присутствует в массиве.`BCEL`Пример:Код банка. |
| { 	"type": "add_info", 	"title": "account_number", 	"data": "123 456 789" } | display_dataВсегда присутствует в массиве.`123 456 789`Пример:Номер счета получателя. |
| { 	"type": "add_info", 	"title": "bank_name", 	"data": "BCEL" } | display_dataВсегда присутствует в массиве.`BCEL`Пример:Название банка. |
...
| language_codetype_string type_optional | `en``en`Пример:Код языка сообщения о результатах оплаты. По умолчанию сообщения передаются на лаосском языке.  Чтобы изменить язык сообщений на английский, передайте значениев этом параметре. |
...
| **Особенности**Если пользователь выполнит перевод на сумму, отличную от суммы, указанной в запросе на оплату, то платеж будет отклонен. |  |
