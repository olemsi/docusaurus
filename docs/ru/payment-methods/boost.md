...
| Конечная точка | `/v2/payment/wallet/boost/sale` |
...
| --- | --- |
| Формат тела запроса | JSON |
| Конечная точка | `/v2/payment/wallet/boost/payout` |
| Полная спецификация конечной точки |  |
...
| **Возврат** |  |
| **Организация и стоимость подключения** | По согласованию с курирующим менеджером Some Platform |
| **Особенности**Актуальность следующих особенностей уточняйте у курирующего менеджера Some Platform.Если пользователь при оплате укажет сумму, отличную от суммы, указанной в запросе на оплату, то платеж будет отклонен.Платеж отклоняется, если пользователь не совершит оплату в течение 15 минут.#ru_pm_boost_purchase_redirect/section_jjc_trv_35bВ некоторых случаях платежная платформа может увеличить сумму платежа на величину от 0,01 до 0,99 MYR. Подробнее о причинах и механизме изменения суммы в разделе |  |
