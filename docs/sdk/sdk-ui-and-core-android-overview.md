...
Mobile sdk_ui_android is an software development kit with open-source code that can be used to integrate Android applications with the Some Platform payment platform.

sdk_ui_android provides the functionality for interaction of customers with the user interface and for interaction of a mobile application with the payment platform which allows sending and receiving necessary information during payment processing. Additionally, the open-source code available with sdk_ui_android provides flexibility for configuring the user interface in accordance with the aspects of the application.

sdk_ui_android can be embedded in mobile applications developed for Android version 5.0 or later. The libraries and code examples are available on GitHub:

- List of sdk_ui_android releases:
- Code examples:

## Capabilities

The following functional capabilities are supported by sdk_ui_android:

- One-time one-step purchasesOne-time two-step purchases (an authorization hold can be placed via the SDK and subsequent debiting of the authorized amount is carried out via Gate or Dashboard).COF purchases (they can be registered via the SDK and then managed via Gate or Dashboard).For card and Google Pay payments, the payment interface described in this section is used. With other payment methods, Payment Widget is used during payment processing.Processing different types of payments made with cards and Google Pay as well as other payment methods available for the merchant's project. Supported payment types include:
...
Generally, the following workflow is relevant when one-step purchases are processed with the use of sdk_ui_android.
...
2. In the mobile application, a set of parameters for creating a payment session is generated. Then, with the help of sdk_ui_android, this set is converted into a string for signing, and the string is sent to the server side of the merchant web service.
3. On the server side of the merchant web service, the parameters can be checked and supplemented if necessary, and the signature to the final parameter set is generated, following which the prepared data is sent back to sdk_ui_android.
4. With the help of sdk_ui_android, a payment session is initiated in the payment platform.
...
8. sdk_ui_android sends a purchase request to the payment platform.
...
12. The information about the purchase result is sent from the payment platform to sdk_ui_android.
13. The notification with the result information is displayed to the customer in the user interface.

## User interface

When card and Google Pay payments are processed, the customer uses with the user interface designed by the Some Platform specialists. Though, you can customized the user interface by changing colors and adding your company's logo.
