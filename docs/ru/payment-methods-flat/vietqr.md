...
| Элемент | Описание |
| --- | --- |
| {     "type": "qr_data",     "title": "QR code",     "data": "00020101com.359a71a-0cad-4cb2-a937-a3fa5319a5252070503***63042BBF" } | `00020101com.359a71a-0cad-4cb2-a937-a3fa5319a5252070503***63042BBF`Пример:Данные, которые нужно отобразить в виде QR-кода. |

| Элемент | Описание |
| --- | --- |
| {     "type": "qr_url",     "title": "qr_img",     "data": "http://example.com/images/qr-code.png" } | `http://example.com/images/qr-code.png`Пример:Ссылка, по которой отображается QR-код. |
| {     "type": "add_info",     "title": "QR Code Timeout",     "data": "1701851362" } | `1701851362`Пример:Время истечения таймера для QR-кода в формате времени Unix. |
...
ru_pm_vietqr_gate_purchase_redirect.ditaПодробная информация о проведении оплаты с использованием метода title через Gate представлена в разделе.
...
ru_pm_vietqr_pp_purchase_redirect.ditaПодробная информация о проведении оплаты с использованием метода title через Payment Widget представлена в разделе.
...
## VietQR -Оплата 2: сканирование QR-кода
...
| **Особенности**references/ru/currencies/VND.ditareferences/ru/currencies/VND.dita`1000`Например, при оплате на 1000в запросе нужно передавать значение.ru_currency_codes.ditaЧисло дробных единиц для различных валют указано в разделе.Суммы в запросах с использованием валютыуказываются в основных единицах валюты в соответствии со стандартом ISO-4217 alpha-3.Оплату с использованием платежного метода VietQR пользователь может выполнять без комиссии, имея счет в любом вьетнамском банке.ru_gate_amount_modification.ditaЕсли пользователь при оплате укажет сумму, отличную от суммы, указанной в запросе на оплату, то платеж будет проводиться с особенностями, которые описаны в разделе. |  |

---

## VietQR -Оплата 1: перенаправление в сервис провайдера
...
| **Возврат** |  |
| **Организация и стоимость подключения** | По согласованию с курирующим менеджером Some Platform |
| **Особенности**references/ru/currencies/VND.ditareferences/ru/currencies/VND.dita`1000`Например, при оплате на 1000в запросе нужно передавать значение.ru_currency_codes.ditaЧисло дробных единиц для различных валют указано в разделе.Суммы в запросах с использованием валютыуказываются в основных единицах валюты в соответствии со стандартом ISO-4217 alpha-3.Оплату с использованием платежного метода VietQR пользователь может выполнять без комиссии, имея счет в любом вьетнамском банке.ru_gate_amount_modification.ditaЕсли пользователь при оплате укажет сумму, отличную от суммы, указанной в запросе на оплату, то платеж будет проводиться с особенностями, которые описаны в разделе.Для индивидуальной настройки данного времени, обратитесь к специалистам технической поддержки Some Platform (support@example.com отклоняется, если пользователь не совершит оплату в течение определенного времени с момента его перенаправления в сервис provider_service. Таймер на платежной форме отображает оставшееся для выполнения платежа время. |  |
