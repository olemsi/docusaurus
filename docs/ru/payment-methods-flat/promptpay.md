...
| Конечная точка | `/v2/payment/banks/promptpay/sale` |
...
| accounttype_required✱type_object | numbertype_required✱type_string | `123456`Пример:Номер счета пользователя. |
| bank_idtype_required✱type_integer | `61021`Пример:Идентификатор банка пользователя. Для получения информации об идентификаторах доступных банков обратитесь к специалистам технической поддержки Some Platform (support@example.com |  |
...
"display_data": [
    {
        "type": "qr_img",
        "title": "QR code",
        "data": "data:image//png;base64,iVBORw0KGgo...CXBIWXMAAA7EAAA"
    },
    {
        "type": "add_info",
...
| Элемент | Описание |
| --- | --- |
| {     "type": "qr_img",     "title": "QR code",     "data": "data:image//png;base64,iVBORw0KGgo...CXBIWXMAAA7EAAA" } | `data:image//png;base64,iVBORw0KGgo...CXBIWXMAAA7EAAA`Пример:Элемент массива с данными для отображения QR-кода пользователю. |
| {     "type": "add_info",     "title": "expired_timestamp",     "data": "600" } | `600`Пример:Элемент массива с информацией о сроке действия QR-кода. |
...
ru_pm_promptpay_gate_purchase_redirect.ditaПодробная информация о проведении оплаты с использованием метода title через Gate представлена в разделе.

- **тестового**в запросах вы должны передавать идентификаторпроекта;
- references/ru/currencies/THB.ditaв качестве валюты оплаты в запросе можно использовать только;
...
ru_pm_promptpay_pp_purchase_redirect.ditaПодробная информация о проведении оплаты с использованием метода title через Payment Widget представлена в разделе.
...
## PromptPay -Оплата 2: сканирование QR-кода
...
## PromptPay -Оплата 1: перенаправление в сервис провайдера
...
| **Возврат** |  |
| **Организация и стоимость подключения** | По согласованию с курирующим менеджером Some Platform |
| **Особенности**QR-код для оплаты должен использоваться только один раз. Для повторной оплаты пользователь должен инициировать новый платеж.После сканирования QR-кода отображаемая сумма оплаты может отличаться от суммы, которая была передана в запросе. Пользователь должен совершить оплату на отображаемую ему сумму и вам также будет зачислена эта сумма.ru_gate_amount_modification.ditaЕсли пользователь при оплате укажет сумму, отличную от суммы, указанной в запросе на оплату, то платеж будет проводиться с особенностями, которые описаны в разделе. Для получения более подробной информации обратитесь к своему курирующему менеджеру Some Platform.Актуальность следующих особенностей уточняйте у курирующего менеджера Some Platform. |  |
...
| title — это платежный метод для проведения оплаты в Таиланде с помощью сканирования QR-кодов с использованием мобильных приложений таиландских банков.Платежный метод title позволяет принимать от пользователей оплату, используя Payment Widget и Gate.здесьЗагрузить логотип этого платежного метода в векторном формате можно. |  |
| --- | --- |
