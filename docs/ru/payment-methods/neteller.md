...
| Конечная точка | `/v2/payment/wallet/neteller/sale` |
...
| addresstype_strictly_required✱type_string | `4 Privet Drive`Пример:Адрес пользователя (улица, номер дома, номер корпуса, номер квартиры и т.п.). |  |
| citytype_strictly_required✱type_string | `Little Whinging`Пример:Город пользователя. |  |
| countrytype_strictly_required✱type_string | `GB`Пример:Страна пользователя в формате ISO 3166-1 alpha-2. |  |
| ziptype_strictly_required✱type_string | `123456`Пример:Почтовый индекс пользователя. |  |
| accounttype_object type_strictly_required | numbertype_string type_strictly_required | `johndoe@example.com`Пример:Адрес электронной почты, на которую зарегистрирована учетная запись пользователя в сервисе title. В соответствии с протоколом работы сервиса title требуется настроить проверку вводимых пользователями значений на стороне веб-сервиса и допускать ввод только таких значений, которые соответствуют формату адреса электронной почты. |
...
| customer_phonetype_strictly_required✱type_string | `12345678901`Пример:Номер телефона пользователя. |
| customer_account_numbertype_string type_required | `johndoe@example.com`Пример:Адрес электронной почты, на которую зарегистрирована учетная запись пользователя в сервисе title. |
...
| customer_countrytype_strictly_required✱type_string | `GB`Пример:Страна пользователя в формате ISO 3166-1 alpha-2. |
| customer_citytype_strictly_required✱type_string | `Little Whinging`Пример:Город пользователя. |
| customer_ziptype_strictly_required✱type_string | `123456`Пример:Почтовый индекс пользователя. |
| customer_addresstype_strictly_required✱type_string | `4 Privet Drive`Пример:Адрес пользователя (улица, номер дома, номер корпуса, номер квартиры и т.п.). |
...
| **Регионы использования** | Все страны, кроме:,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,. |
| --- | --- |
| **Валюты оплаты** | Информацию уточняйте у курирующего менеджера Some Platform. |
...
| **Особенности**ru_pp_method_embedded.ditaСпособ открытия Payment Widget — любой, кроме. |  |
...
| --- | --- |
| Регионы использования | Все страны, кроме:,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,. |
| Валюты платежей | Информацию уточняйте у курирующего менеджера Some Platform. |
| Суммы платежей | Информацию уточняйте у курирующего менеджера Some Platform. Также вы можете уточнить в Dashboard минимальную и максимальную сумму платежа, доступную в вашем проекте. |
| Время проведения платежа | Информацию уточняйте у курирующего менеджера Some Platform. |
| Конвертация валют | На стороне Some Platform |
| Оплата |  |
